...
import org.thalm.game.monster.Monster;
...
import org.thalm.game.npc.NonPlayerCharacter.*;
import org.thalm.game.spells.Spell;
...
public class World {
	...
	private static Map<Integer,Maps> 	Cartes	= new TreeMap<Integer,Maps>();
	private static Map<Integer,Item> 	Objets	= new TreeMap<Integer,Item>();
	...
	private static Map<Integer,NonPlayerCharacter> NPCTemplates = new TreeMap<Integer,NonPlayerCharacter>();
	...
	private static Map<Integer,ActionHouse> Hdvs = new TreeMap<Integer,ActionHouse>();
	...
	public static class SubArea
	{
		...
		private String _name;
		private ArrayList<Maps> _maps = new ArrayList<Maps>();
	...
	public static class Exchange
	{
		...
		synchronized public void toogleOK(int guid)
		{
			...
			{
				ok1 = !ok1;
				Packets.GAME_SEND_EXCHANGE_OK(perso1.get_compte().getGameThread().get_out(),ok1,guid);
				Packets.GAME_SEND_EXCHANGE_OK(perso2.get_compte().getGameThread().get_out(),ok1,guid);
			}
			else if (i == 2)
			{
				ok2 = !ok2;
				Packets.GAME_SEND_EXCHANGE_OK(perso1.get_compte().getGameThread().get_out(),ok2,guid);
				Packets.GAME_SEND_EXCHANGE_OK(perso2.get_compte().getGameThread().get_out(),ok2,guid);
			}
			else 
				return;
		...
		synchronized public void setKamas(int guid, long k)
		{
			...
			if(perso1.get_GUID() == guid)
				...
				i = 2;
			Packets.GAME_SEND_EXCHANGE_OK(perso1.get_compte().getGameThread().get_out(),ok1,perso1.get_GUID());
			Packets.GAME_SEND_EXCHANGE_OK(perso2.get_compte().getGameThread().get_out(),ok1,perso1.get_GUID());
			Packets.GAME_SEND_EXCHANGE_OK(perso1.get_compte().getGameThread().get_out(),ok2,perso2.get_GUID());
			Packets.GAME_SEND_EXCHANGE_OK(perso2.get_compte().getGameThread().get_out(),ok2,perso2.get_GUID());
			
			if(i == 1)
			{
				kamas1 = k;
				Packets.GAME_SEND_EXCHANGE_MOVE_OK(perso1, 'G', "", k+"");
				Packets.GAME_SEND_EXCHANGE_OTHER_MOVE_OK(perso2.get_compte().getGameThread().get_out(), 'G', "", k+"");
			}else if (i == 2)
			{
				kamas2 = k;
				Packets.GAME_SEND_EXCHANGE_OTHER_MOVE_OK(perso1.get_compte().getGameThread().get_out(), 'G', "", k+"");
				Packets.GAME_SEND_EXCHANGE_MOVE_OK(perso2, 'G', "", k+"");	
			}
		}
		
		synchronized public void cancel()
		{
			if(perso1.get_compte() != null)if(perso1.get_compte().getGameThread() != null)Packets.GAME_SEND_EV_PACKET(perso1.get_compte().getGameThread().get_out());
			if(perso2.get_compte() != null)if(perso2.get_compte().getGameThread() != null)Packets.GAME_SEND_EV_PACKET(perso2.get_compte().getGameThread().get_out());
		...
		synchronized public void apply()
		{
			...
			for(Couple<Integer, Integer> couple : items1)
			{
				...
				if((obj.getQuantity() - couple.second) <1)//S'il ne reste plus d'item apres l'échange
				{
					...
					Packets.GAME_SEND_REMOVE_ITEM_PACKET(perso1, couple.first);
				...
				}else
				{
					...
					Packets.GAME_SEND_OBJECT_QUANTITY_PACKET(perso1, obj);
			...
			for(Couple<Integer, Integer> couple : items2)
			{
				...
				if((obj.getQuantity() - couple.second) <1)//S'il ne reste plus d'item apres l'échange
				{
					...
					Packets.GAME_SEND_REMOVE_ITEM_PACKET(perso2, couple.first);
				...
				}else
				{
					...
					Packets.GAME_SEND_OBJECT_QUANTITY_PACKET(perso2, obj);
					Item newObj = Item.getCloneObjet(obj, couple.second);
			...
			perso1.setCurExchange(null);
			perso2.setCurExchange(null);
			Packets.GAME_SEND_Ow_PACKET(perso1);
			Packets.GAME_SEND_Ow_PACKET(perso2);
			Packets.GAME_SEND_STATS_PACKET(perso1);
			Packets.GAME_SEND_STATS_PACKET(perso2);
			Packets.GAME_SEND_EXCHANGE_VALID(perso1.get_compte().getGameThread().get_out(),'a');
			Packets.GAME_SEND_EXCHANGE_VALID(perso2.get_compte().getGameThread().get_out(),'a');	
			Database.SAVE_PERSONNAGE(perso1,true);
			Database.SAVE_PERSONNAGE(perso2,true);
		}
		...
		synchronized public void addItem(int guid, int qua, int pguid)
		{
			...
			if(i == 1)
			{
				...
				{
					couple.second += qua;
					Packets.GAME_SEND_EXCHANGE_MOVE_OK(perso1, 'O', "+", ""+guid+"|"+couple.second);
					Packets.GAME_SEND_EXCHANGE_OTHER_MOVE_OK(perso2.get_compte().getGameThread().get_out(), 'O', "+", ""+guid+"|"+couple.second+add);
				return;
				}
				items1.add(new Couple<Integer,Integer>(guid,qua));
				Packets.GAME_SEND_EXCHANGE_MOVE_OK(perso1, 'O', "+", str);
				Packets.GAME_SEND_EXCHANGE_OTHER_MOVE_OK(perso2.get_compte().getGameThread().get_out(), 'O', "+", str+add);	
			}else if(i == 2)
			{
				...
				{
					couple.second += qua;
					Packets.GAME_SEND_EXCHANGE_OTHER_MOVE_OK(perso1.get_compte().getGameThread().get_out(), 'O', "+", ""+guid+"|"+couple.second+add);
					Packets.GAME_SEND_EXCHANGE_MOVE_OK(perso2, 'O', "+", ""+guid+"|"+couple.second);
				return;
				}
				items2.add(new Couple<Integer,Integer>(guid,qua));
				Packets.GAME_SEND_EXCHANGE_MOVE_OK(perso2, 'O', "+", str);
				Packets.GAME_SEND_EXCHANGE_OTHER_MOVE_OK(perso1.get_compte().getGameThread().get_out(), 'O', "+", str+add);	
			}
		}
		...
		synchronized public void removeItem(int guid, int qua, int pguid)
		{
			...
			if(i == 1)
			{
				...
				if(newQua <1)//Si il n'y a pu d'item
				{
					...
					Packets.GAME_SEND_EXCHANGE_MOVE_OK(perso1, 'O', "-", ""+guid);
					Packets.GAME_SEND_EXCHANGE_OTHER_MOVE_OK(perso2.get_compte().getGameThread().get_out(), 'O', "-", ""+guid);
				}else
				{
					couple.second = newQua;
					Packets.GAME_SEND_EXCHANGE_MOVE_OK(perso1, 'O', "+", ""+guid+"|"+newQua);
					Packets.GAME_SEND_EXCHANGE_OTHER_MOVE_OK(perso2.get_compte().getGameThread().get_out(), 'O', "+", ""+guid+"|"+newQua+add);
				}
			}else if(i ==2)
			{
				Couple<Integer,Integer> couple = getCoupleInList(items2,guid);
				...
				if(newQua <1)//Si il n'y a pu d'item
				{
					...
					Packets.GAME_SEND_EXCHANGE_OTHER_MOVE_OK(perso1.get_compte().getGameThread().get_out(), 'O', "-", ""+guid);
					Packets.GAME_SEND_EXCHANGE_MOVE_OK(perso2, 'O', "-", ""+guid);
				}else
				{
					couple.second = newQua;
					Packets.GAME_SEND_EXCHANGE_OTHER_MOVE_OK(perso1.get_compte().getGameThread().get_out(), 'O', "+", ""+guid+"|"+newQua+add);
					Packets.GAME_SEND_EXCHANGE_MOVE_OK(perso2, 'O', "+", ""+guid+"|"+newQua);
				}
			}
		}
	...
	public static void createWorld()
	{
		...
		Database.LOAD_EXP();
		Main.MAX_LEVEL = ExpLevels.size();	//MARTHIEUBEAN
	...
	public static NonPlayerCharacter getNPCTemplate(int guid)
	{
		return NPCTemplates.get(guid);
	}
	
	public static void addNpcTemplate(NonPlayerCharacter temp)
	{
		NPCTemplates.put(temp.get_id(), temp);
	}
	
	public static Maps getCarte(int id)
	{
	...
	{
		Database.DELETE_PERSO_IN_BDD(perso);
		World.unloadPerso(perso.get_GUID());
	}
	
	public static long getPersoXpMin(int _lvl)
	{
		if(_lvl > Main.MAX_LEVEL) 	_lvl = Main.MAX_LEVEL;
		if(_lvl < 1) 	_lvl = 1;
	...
	{
		if(_lvl >= Main.MAX_LEVEL) 	_lvl = Main.MAX_LEVEL-1;
		if(_lvl <= 1)	 	_lvl = 1;
	...
	public static void saveAll(Player saver)
	{
		...
		try
		{
			...
			Database.commitTransacts();
			Database.TIMER(false);	//Arrête le timer d'enregistrement SQL
			
			GameServer.addToLog("Sauvegarde des personnages...");
			for(Player perso : Persos.values())
			{
				if(!perso.isOnline())continue;
				Database.SAVE_PERSONNAGE(perso,true);//sauvegarde des persos et de leurs items
		...
		}catch(ConcurrentModificationException e)
		{
			if(saveTry < 10)
			{
				...
				if(saver != null && _out != null)
					Packets.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, "Erreur. Nouvelle tentative de sauvegarde");
				saveTry++;
				saveAll(saver);
			}
			else
			{
				...
				if(saver != null && _out != null)
					...
					Packets.GAME_SEND_CONSOLE_MESSAGE_TO_ADMIN(mess, 5);
		...
		{
			Database.commitTransacts();
			Database.TIMER(true); //Redémarre le timer d'enregistrement SQL
			Main.isSaving = false;
			saveTry = 1;
		}
	}
	...
	{
		for(Account C : Comptes.values())if(C.get_pseudo().equals(p))return C;
		return null;
	}
	...
	public static void addGuild(Guild g,boolean save)
	{
		...
		if(save)Database.SAVE_NEWGUILD(g);
	}
	...
	{
		C.get_persos().clear();
		Database.LOAD_PERSO_BY_ACCOUNT(C.get_GUID());
	...
	{
		for(int[] zaap : Variable.AMAKNA_ZAAPS)
		{
			if(zaap[0] == i)return zaap[1];
		}
		for(int[] zaap : Variable.INCARNAM_ZAAPS)
		{
			if(zaap[0] == i)return zaap[1];
		}
		return -1;
	...
	{
		for(int[] zaapi : Variable.BONTA_ZAAPI)
		{
			if(zaapi[0] == i)return zaapi[1];
		}
		for(int[] zaapi : Variable.BRAKMAR_ZAAPI)
		{
			if(zaapi[0] == i)return zaapi[1];
		}
		return -1;
	...
	{
		Guildes.remove(id);
		Database.DEL_GUILD(id);
	}

	public static int ipIsUsed(String ip)
	{
		int used = 0;
		for(Account c : Comptes.values())
	...
	public static void addHdv(ActionHouse toAdd)
	{
		Hdvs.put(toAdd.getHdvID(),toAdd);
	}
	
	public static ActionHouse getHdv(int mapID)
	{
	...
	public static void addKamasToAcc(int compteID,int amount)
	{
		...
		{
			Account toAdd = getCompte(compteID);
			toAdd.addBankKamas(amount);
		}
		else
		{
			Database.UPDATE_ACCOUNT_BANKKAMAS(compteID,amount);
		}
	}
	...
	public static void moveEntities() {
		synchronized(Cartes.values())
		{
			for(final Maps curCarte : Cartes.values())
			{
				if(curCarte != null)
				{
					final char[] dirs = {'b','d','f','h'};
					if(curCarte.get_npcs() != null && curCarte.get_npcs().size() > 0) {
						NPC npc = curCarte.get_npcs().get(Utils.getRandomValue(0, curCarte.get_npcs().size()-1));
						if(npc != null) {
							final char dir = dirs[Utils.getRandomValue(0, 3)];
							final Couple<Integer, String> couplePath = Pathfinding.getSpecialPath(curCarte, 
									dir, npc.get_cellID(), Utils.getRandomValue(2, 8));
							
							Packets.GAME_SEND_GA_PACKET_TO_MAP(curCarte, "0", 1, "", 
									"a"+Cipher.getHashedValueByInt(npc.get_cellID())+couplePath.second);
							
							npc.setCellID(couplePath.first);
							npc.setOrientation((byte)Pathfinding.getOrientationFromDir(dir));
						}
					}
				}
			}
