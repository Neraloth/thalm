...
import org.thalm.game.fight.Fight.Glyphe;
import org.thalm.game.fight.Fight.Piege;
...
public class SpellEffect
	{
		...
		{
			for(int id : Variable.ON_HIT_BUFFS)
			{
				for(SpellEffect buff : target.getBuffsByEffectID(id))
				{
					switch(id)
					{
						case 9://Derobade
							...
							int c = Utils.getRandomValue(0, 99);
							...
							Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 5, target.getGUID()+"", target.getGUID()+","+newCellID);
						...
						case 79://chance éca
							try
							{
								...
								int jet = Utils.getRandomValue(0, 99);
						...
						case 107://renvoie Dom
							...
							float coef = 1+(target.getTotalStats().getEffect(Variable.STATS_ADD_SAGE)/100);
							int renvoie = 0;
							try
							{
							...
							Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 107, "-1", target.getGUID()+","+renvoie);
							if(renvoie>caster.getPDV())renvoie = caster.getPDV();
							if(finalDommage<0)finalDommage =0;
							caster.removePDV(renvoie);
							Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 100, caster.getGUID()+"", caster.getGUID()+",-"+renvoie);
						break;
						...
						case 788://Chatiments
							...
							Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, stat, caster.getGUID()+"", target.getGUID()+","+gain+","+5);
		...
		private void applyEffect_51(Fight fight)
		{
			...
			target.get_fightCell().addFighter(target);
			//on enleve les états
			target.setState(Variable.ETAT_PORTE, 0);
			caster.setState(Variable.ETAT_PORTEUR, 0);
			//on dé-lie les 2 Fighter
			target.set_holdedBy(null);
			caster.set_isHolding(null);
			
			//on envoie les packets
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 51, caster.getGUID()+"", cell.getID()+"");
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 950, target.getGUID()+"", target.getGUID()+","+Variable.ETAT_PORTE+",0");
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 950, caster.getGUID()+"", caster.getGUID()+","+Variable.ETAT_PORTEUR+",0");
		}
		...
		private void applyEffect_950(Fight fight, ArrayList<Fighter> cibles)
		{
			...
			for(Fighter target : cibles)
			{
				...
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 950, caster.getGUID()+"", target.getGUID()+","+id+",1");
			}
		}
		...
		private void applyEffect_951(Fight fight, ArrayList<Fighter> cibles)
		{
			...
			for(Fighter target : cibles)
			{
				...
				//on envoie le packet
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 950, caster.getGUID()+"", target.getGUID()+","+id+",0");
			}
		}
		...
		private void applyEffect_50(Fight fight)
		{
			...
			target.setState(Variable.ETAT_PORTE, -1);
			caster.setState(Variable.ETAT_PORTEUR, -1);
			//on lie les 2 Fighter
			target.set_holdedBy(caster);
			caster.set_isHolding(target);
			
			//on envoie les packets
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 950, target.getGUID()+"", target.getGUID()+","+Variable.ETAT_PORTE+",1");
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 950, caster.getGUID()+"", caster.getGUID()+","+Variable.ETAT_PORTEUR+",1");
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 50, caster.getGUID()+"", ""+target.getGUID());
		}
		...
		private void applyEffect_185(Fight fight)
		{
			...
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 181, caster.getGUID() + "", gm);
		...
		private void applyEffect_672(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			double val = ((double)Utils.getRandomJet(jet)/(double)100);
			...
			for(Fighter target : cibles)
			{
				...
				{
					Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 106, target.getGUID()+"", target.getGUID()+",1");
				...
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 100, caster.getGUID()+"", target.getGUID()+","+finalDommage);
				if(target.getPDV() <=0)
		...
		private void applyEffect_783(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 5, caster.getGUID()+"", target.getGUID()+","+cell.getID());
		}
		...
		private void applyEffect_8(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 4, caster.getGUID()+"", target.getGUID()+","+exCaster.getID());
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 4, caster.getGUID()+"", caster.getGUID()+","+exTarget.getID());
			
		}

		private void applyEffect_266(Fight fight, ArrayList<Fighter> cibles)
		{
			int val = Utils.getRandomJet(jet);
			int vol = 0;
			for(Fighter target : cibles)
			{
				target.addBuff(Variable.STATS_REM_CHAN, val, turns, 1, false, spell, args, caster);
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Variable.STATS_REM_CHAN, caster.getGUID()+"", target.getGUID()+","+val+","+turns);
				vol += val;
			}
			if(vol == 0)return;
			//on ajoute le buff
			caster.addBuff(Variable.STATS_ADD_CHAN, vol, turns, 1, false, spell, args, caster);
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Variable.STATS_ADD_CHAN, caster.getGUID()+"", caster.getGUID()+","+vol+","+turns);
		}
		...
		private void applyEffect_267(Fight fight, ArrayList<Fighter> cibles)
		{
			...
			for(Fighter target : cibles)
			{
				target.addBuff(Variable.STATS_REM_VITA, val, turns, 1, false, spell, args, caster);
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Variable.STATS_REM_VITA, caster.getGUID()+"", target.getGUID()+","+val+","+turns);
				vol += val;
			}
			if(vol == 0)return;
			//on ajoute le buff
			caster.addBuff(Variable.STATS_ADD_VITA, vol, turns, 1, false, spell, args, caster);
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Variable.STATS_ADD_VITA, caster.getGUID()+"", caster.getGUID()+","+vol+","+turns);
		}
		...
		private void applyEffect_268(Fight fight, ArrayList<Fighter> cibles)
		{
			...
			for(Fighter target : cibles)
			{
				target.addBuff(Variable.STATS_REM_AGIL, val, turns, 1, false, spell, args, caster);
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Variable.STATS_REM_AGIL, caster.getGUID()+"", target.getGUID()+","+val+","+turns);
				vol += val;
			}
			if(vol == 0)return;
			//on ajoute le buff
			caster.addBuff(Variable.STATS_ADD_AGIL, vol, turns, 1, false, spell, args, caster);
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Variable.STATS_ADD_AGIL, caster.getGUID()+"", caster.getGUID()+","+vol+","+turns);
		}
		...
		private void applyEffect_269(Fight fight, ArrayList<Fighter> cibles)
		{
			...
			for(Fighter target : cibles)
			{
				target.addBuff(Variable.STATS_REM_INTE, val, turns, 1, false, spell, args, caster);
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Variable.STATS_REM_INTE, caster.getGUID()+"", target.getGUID()+","+val+","+turns);
				vol += val;
			}
			if(vol == 0)return;
			//on ajoute le buff
			caster.addBuff(Variable.STATS_ADD_INTE, vol, turns, 1, false, spell, args, caster);
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Variable.STATS_ADD_INTE, caster.getGUID()+"", caster.getGUID()+","+vol+","+turns);
		}
		...
		private void applyEffect_270(Fight fight, ArrayList<Fighter> cibles)
		{
			...
			for(Fighter target : cibles)
			{
				target.addBuff(Variable.STATS_REM_SAGE, val, turns, 1, false, spell, args, caster);
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Variable.STATS_REM_SAGE, caster.getGUID()+"", target.getGUID()+","+val+","+turns);
				vol += val;
			}
			if(vol == 0)return;
			//on ajoute le buff
			caster.addBuff(Variable.STATS_ADD_SAGE, vol, turns, 1, false, spell, args, caster);
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Variable.STATS_ADD_SAGE, caster.getGUID()+"", caster.getGUID()+","+vol+","+turns);
		}
		...
		private void applyEffect_271(Fight fight, ArrayList<Fighter> cibles)
		{
			...
			for(Fighter target : cibles)
			{
				target.addBuff(Variable.STATS_REM_FORC, val, turns, 1, false, spell, args, caster);
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Variable.STATS_REM_FORC, caster.getGUID()+"", target.getGUID()+","+val+","+turns);
				vol += val;
			}
			if(vol == 0)return;
			//on ajoute le buff
			caster.addBuff(Variable.STATS_ADD_FORC, vol, turns, 1, false, spell, args, caster);
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Variable.STATS_ADD_FORC, caster.getGUID()+"", caster.getGUID()+","+vol+","+turns);
		}
		...
		private void applyEffect_265(Fight fight, ArrayList<Fighter> cibles)
		{
			...
			{
				target.addBuff(effectID, val, turns, 1, false, spell, args, caster);
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, effectID, caster.getGUID()+"", target.getGUID()+","+val+","+turns);
			}
		}
		...
		private void applyEffect_149(final Fight fight, final ArrayList<Fighter> cibles)
		{
		        ...
		        for(final Fighter target : cibles)
		        {
		                if(spell == 686)
		                {
		                        if(target.getPersonnage().get_sexe() == 1 && target.getMob().getTemplate().getID() == 547)
		                        {
		                                id = 8011;
		                        }
		                }
		                ...
		                Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, effectID, caster.getGUID()+"", target.getGUID()+","+defaut+","+id+","+turns);
		        }      
		}
		...
		private void applyEffect_150(Fight fight, ArrayList<Fighter> cibles)
		{
			...
			for(Fighter target : cibles)
			{
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 150, caster.getGUID()+"", target.getGUID()+",4");
		...
		private void applyEffect_401(Fight fight)
		{
			...
			byte size = (byte) Cipher.getIntByHashedValue(po.charAt(1));
			...
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 999, caster.getGUID()+"", str);
		...
		private void applyEffect_400(Fight fight)
		{
			...
			String str = "GDZ+"+cell.getID()+";"+size+";"+unk;
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, team, 999, caster.getGUID()+"", str);
		...
		private void applyEffect_181(Fight fight)//invocation
		{
			...
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 181, caster.getGUID() + "", gm);
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 999, caster.getGUID()+"", gtl);
		...
		private void applyEffect_77(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			for(Fighter target : cibles)
			{
				int val = Utils.getPointsLost('m', value, caster, target);
				if(val < value)
				{
					Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 309, caster.getGUID()+"", target.getGUID()+","+(value-val));
				}
				if(val < 1)continue;
				target.addBuff(Variable.STATS_REM_PM, val, turns,0, true, spell,args,caster);
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,Variable.STATS_REM_PM, caster.getGUID()+"", target.getGUID()+",-"+val+","+turns);
				num += val;
			}
			if(num != 0)
			{
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,Variable.STATS_ADD_PM, caster.getGUID()+"", caster.getGUID()+","+num+","+turns);
				caster.addBuff(Variable.STATS_ADD_PM, num, 1, 0, true, spell,args,caster);
			}
		}
		...
		private void applyEffect_84(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			for(Fighter target : cibles)
			{
				int val = Utils.getPointsLost('m', value, caster, target);
				if(val < value)
					Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 308, caster.getGUID()+"", target.getGUID()+","+(value-val));

				if(val < 1)continue;
				target.addBuff(Variable.STATS_REM_PA, val, turns,0, true, spell,args,caster);
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,Variable.STATS_REM_PA, caster.getGUID()+"", target.getGUID()+",-"+val+","+turns);
				num += val;
			}
			if(num != 0)
			{
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,Variable.STATS_ADD_PA, caster.getGUID()+"", caster.getGUID()+","+num+","+turns);
				caster.addBuff(Variable.STATS_ADD_PA, num, 1, 0, true, spell,args,caster);
			}
		}
		...
		private void applyEffect_168(Fight fight, ArrayList<Fighter> cibles)
		{
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 168,target.getGUID()+"",target.getGUID()+",-"+value);
				}
			}else
			{
				for(Fighter target : cibles)
		...
		private void applyEffect_169(Fight fight, ArrayList<Fighter> cibles)
		{
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					target.addBuff(effectID, value, 1, 1, false, spell, args, caster);
					if(turns <= 1 || duration <= 1)
					Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 169,target.getGUID()+"",target.getGUID()+",-"+value);
				}
			}else
			{
				for(Fighter target : cibles)
		...
		private void applyEffect_101(ArrayList<Fighter> cibles, Fight fight)
		{
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					int retrait = Utils.getPointsLost('a',value,caster,target);
					if((value -retrait) > 0)
						Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 308, caster.getGUID()+"", target.getGUID()+","+(value-retrait));
					if(retrait > 0)
					{
						target.addBuff(Variable.STATS_REM_PA, retrait, 1, 1, false, spell, args, caster);
						if(turns <= 1 || duration <= 1)
							Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 101,target.getGUID()+"",target.getGUID()+",-"+retrait);
					}
				}
			}else
			{
		...
		private void applyEffect_127(ArrayList<Fighter> cibles, Fight fight)
		{
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					int retrait = Utils.getPointsLost('m',value,caster,target);
					if((value -retrait) > 0)
						Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 309, caster.getGUID()+"", target.getGUID()+","+(value-retrait));
					if(retrait > 0)
					{
						target.addBuff(Variable.STATS_REM_PM, retrait, 1, 1, false, spell, args, caster);
						if(turns <= 1 || duration <= 1)
							Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 127,target.getGUID()+"",target.getGUID()+",-"+retrait);
					}
				}
			}else
			{
		...
		private void applyEffect_4(Fight fight,ArrayList<Fighter> cibles)
		{
			...
			if(cell.isWalkable(true) && !fight.isOccuped(cell.getID()))//Si la case est prise, on va éviter que les joueurs se montent dessus *-*
			{
				...
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 4, caster.getGUID()+"", caster.getGUID()+","+cell.getID());
			}else
			{
		...
		private void applyEffect_109(Fight fight)//Dommage pour le lanceur (fixes)
		{
			if(turns <= 0)
			{
				int dmg = Utils.getRandomJet(args.split(";")[5]);
				int finalDommage = Utils.calculFinalDommage(fight,caster, caster,Variable.ELEMENT_NULL, dmg, false);
				...
				finalDommage = -(finalDommage);
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 100, caster.getGUID()+"", caster.getGUID()+","+finalDommage);
		...
		private void applyEffect_82(ArrayList<Fighter> cibles,Fight fight)
		{
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					int finalDommage = Utils.calculFinalDommage(fight,caster, target,Variable.ELEMENT_NULL, dmg,false);
					...
					Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 100, target.getGUID()+"", caster.getGUID()+","+heal);
		...
		private void applyEffect_6(ArrayList<Fighter> cibles,Fight fight)
		{
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					target.set_fightCell(fight.get_map().getCase(newCellID));
					target.get_fightCell().addFighter(target);
					Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 5, caster.getGUID()+"", target.getGUID()+","+newCellID);
		...
		private void applyEffect_5(ArrayList<Fighter> cibles,Fight fight)
		{
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					{
						int a = -newCellID;
						int coef = Utils.getRandomJet("1d8+8");
		...
		private void applyEffect_91(ArrayList<Fighter> cibles,Fight fight)
		{
			if(turns <= 0)
			{
				...
				for(Fighter target : cibles)
				{
					...
					int dmg = Utils.getRandomJet(args.split(";")[5]);
					int finalDommage = Utils.calculFinalDommage(fight,caster, target,Variable.ELEMENT_EAU, dmg,false);
		...
		private void applyEffect_92(ArrayList<Fighter> cibles,Fight fight)
		{
			...
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					int dmg = Utils.getRandomJet(args.split(";")[5]);
					int finalDommage = Utils.calculFinalDommage(fight,caster, target,Variable.ELEMENT_TERRE, dmg,false);
		...
		private void applyEffect_93(ArrayList<Fighter> cibles,Fight fight)
		{
			...
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					int dmg = Utils.getRandomJet(args.split(";")[5]);
					int finalDommage = Utils.calculFinalDommage(fight,caster, target,Variable.ELEMENT_AIR, dmg,false);
		...
		private void applyEffect_94(ArrayList<Fighter> cibles,Fight fight)
		{
			...
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					int dmg = Utils.getRandomJet(args.split(";")[5]);
					int finalDommage = Utils.calculFinalDommage(fight,caster, target,Variable.ELEMENT_FEU, dmg,false);
		...
		private void applyEffect_95(ArrayList<Fighter> cibles,Fight fight)
		{
			...
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					int dmg = Utils.getRandomJet(args.split(";")[5]);
					int finalDommage = Utils.calculFinalDommage(fight,caster, target,Variable.ELEMENT_NEUTRE, dmg,false);
		...
		private void applyEffect_85(ArrayList<Fighter> cibles,Fight fight)
		{
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					int resP = target.getTotalStats().getEffect(Variable.STATS_ADD_RP_EAU);
					int resF = target.getTotalStats().getEffect(Variable.STATS_ADD_R_EAU);
					if(target.getPersonnage() != null)//Si c'est un joueur, on ajoute les resists bouclier
					{
						resP += target.getTotalStats().getEffect(Variable.STATS_ADD_RP_PVP_EAU);
						resF += target.getTotalStats().getEffect(Variable.STATS_ADD_R_PVP_EAU);
					}
					int dmg = Utils.getRandomJet(args.split(";")[5]);//%age de pdv infligé
					...
					val = -(val);
					Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 100, caster.getGUID()+"", target.getGUID()+","+val);
					if(target.getPDV() <=0)
						fight.onFighterDie(target);
				}
			}else
		...
		private void applyEffect_86(ArrayList<Fighter> cibles,Fight fight)
		{
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					int resP = target.getTotalStats().getEffect(Variable.STATS_ADD_RP_TER);
					int resF = target.getTotalStats().getEffect(Variable.STATS_ADD_R_TER);
					if(target.getPersonnage() != null)//Si c'est un joueur, on ajoute les resists bouclier
					{
						resP += target.getTotalStats().getEffect(Variable.STATS_ADD_RP_PVP_TER);
						resF += target.getTotalStats().getEffect(Variable.STATS_ADD_R_PVP_TER);
					}
					int dmg = Utils.getRandomJet(args.split(";")[5]);//%age de pdv infligé
					int val = caster.getPDV()/100*dmg;//Valeur des dégats
					//retrait de la résist fixe
		...
		private void applyEffect_87(ArrayList<Fighter> cibles,Fight fight)
		{
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					int resP = target.getTotalStats().getEffect(Variable.STATS_ADD_RP_AIR);
					int resF = target.getTotalStats().getEffect(Variable.STATS_ADD_R_AIR);
					if(target.getPersonnage() != null)//Si c'est un joueur, on ajoute les resists bouclier
					{
						resP += target.getTotalStats().getEffect(Variable.STATS_ADD_RP_PVP_AIR);
						resF += target.getTotalStats().getEffect(Variable.STATS_ADD_R_PVP_AIR);
					}
					int dmg = Utils.getRandomJet(args.split(";")[5]);//%age de pdv infligé
					int val = caster.getPDV()/100*dmg;//Valeur des dégats
					//retrait de la résist fixe
		...
		private void applyEffect_88(ArrayList<Fighter> cibles,Fight fight)
		{
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					int resP = target.getTotalStats().getEffect(Variable.STATS_ADD_RP_FEU);
					int resF = target.getTotalStats().getEffect(Variable.STATS_ADD_R_FEU);
					if(target.getPersonnage() != null)//Si c'est un joueur, on ajoute les resists bouclier
					{
						resP += target.getTotalStats().getEffect(Variable.STATS_ADD_RP_PVP_FEU);
						resF += target.getTotalStats().getEffect(Variable.STATS_ADD_R_PVP_FEU);
					}
					int dmg = Utils.getRandomJet(args.split(";")[5]);//%age de pdv infligé
					int val = caster.getPDV()/100*dmg;//Valeur des dégats
					//retrait de la résist fixe
		...
		private void applyEffect_89(ArrayList<Fighter> cibles,Fight fight)
		{
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					int resP = target.getTotalStats().getEffect(Variable.STATS_ADD_RP_NEU);
					int resF = target.getTotalStats().getEffect(Variable.STATS_ADD_R_NEU);
					if(target.getPersonnage() != null)//Si c'est un joueur, on ajoute les resists bouclier
					{
						resP += target.getTotalStats().getEffect(Variable.STATS_ADD_RP_PVP_NEU);
						resF += target.getTotalStats().getEffect(Variable.STATS_ADD_R_PVP_NEU);
					}
					int dmg = Utils.getRandomJet(args.split(";")[5]);//%age de pdv infligé
					int val = caster.getPDV()/100*dmg;//Valeur des dégats
					//retrait de la résist fixe
		...
		private void applyEffect_132(ArrayList<Fighter> cibles,Fight fight)
		{
			...
			{
				target.debuff();
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 132, caster.getGUID()+"", target.getGUID()+"");
			}
		}
		...
		private void applyEffect_90(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			{
				int pAge = Utils.getRandomJet(args.split(";")[5]);
		...
		private void applyEffect_108(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			{					
				...
				for(Fighter target : cibles)
				{
					int finalSoin = Utils.calculFinalDommage(fight,caster, target,Variable.ELEMENT_FEU,  dmg, true);
					...
					Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 100, caster.getGUID()+"", target.getGUID()+",+"+finalSoin);
				}
			}else
			{
				for(Fighter target : cibles)
		...
		private void applyEffect_320(Fight fight, ArrayList<Fighter> cibles)
		{
			...
			for(Fighter target : cibles)
			{
				target.addBuff(Variable.STATS_REM_PO, value, turns,0, true, spell,args,caster);
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,Variable.STATS_REM_PO, caster.getGUID()+"", target.getGUID()+","+value+","+turns);
				num += value;
			}
			if(num != 0)
			{
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,Variable.STATS_ADD_PO, caster.getGUID()+"", caster.getGUID()+","+num+","+turns);
				caster.addBuff(Variable.STATS_ADD_PO, num, 1, 0, true, spell,args,caster);
			}
		}
