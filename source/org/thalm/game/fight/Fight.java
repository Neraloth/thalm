...
import org.thalm.game.World.*;
import org.thalm.game.character.Pathfinding;
...
import org.thalm.game.map.Maps.Case;
import org.thalm.game.monster.AI;
import org.thalm.game.monster.Monster.MobGrade;
...
public class Fight
{
	...
	public static class Piege
	{
		...
		public Piege(Fight fight, Fighter caster, Case cell, byte size, SortStats trapSpell, int spell)
		{
			...
			_color = Variable.getTrapsColor(spell);
		}
		...
		public void desapear()
		{
			...
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(_fight, team, 999, _caster.getGUID()+"", str);
			str = "GDC"+_cell.getID();
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(_fight, team, 999, _caster.getGUID()+"", str);
		}
		...
		public void onTraped(Fighter target)
		{
			...
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(_fight, 7, 307, target.getGUID()+"", str);
	...
	public static class Fighter
	{
		...
		public boolean testIfCC(int tauxCC)
		{
			...
			int agi = getTotalStats().getEffect(Variable.STATS_ADD_AGIL);
			if(agi <0)agi =0;
			tauxCC -= getTotalStats().getEffect(Variable.STATS_ADD_CC);
			tauxCC = (int)((tauxCC * 2.9901) / Math.log(agi +12));//Influence de l'agi
			if(tauxCC<2)tauxCC = 2;
			int jet = Utils.getRandomValue(1, tauxCC);
			return (jet == tauxCC);
		}
		...
		public String getGmPacket()
		{
			...
			switch(_type)
			{
				case 1://Perso
					...
					str+= getPDV()+";";
					str+= getTotalStats().getEffect(Variable.STATS_ADD_PA)+";";
					str+= getTotalStats().getEffect(Variable.STATS_ADD_PM)+";";
					str+= getTotalStats().getEffect(Variable.STATS_ADD_RP_NEU)+";";
					str+= getTotalStats().getEffect(Variable.STATS_ADD_RP_TER)+";";
					str+= getTotalStats().getEffect(Variable.STATS_ADD_RP_FEU)+";";
					str+= getTotalStats().getEffect(Variable.STATS_ADD_RP_EAU)+";";	
					str+= getTotalStats().getEffect(Variable.STATS_ADD_RP_AIR)+";";
					str+= getTotalStats().getEffect(Variable.STATS_ADD_AFLEE)+";";
					str+= getTotalStats().getEffect(Variable.STATS_ADD_MFLEE)+";";
					str+= _team+";";
		...
		public void decrementStates()
		{
			...
			for(Entry<Integer,Integer> e : entries)
			{
				...
				if(nVal == 0)//ne pas mettre plus petit, -1 = infinie
				{
					...
					Packets.GAME_SEND_GA_PACKET_TO_FIGHT(_fight, 7, 950, getGUID()+"", getGUID()+","+e.getKey()+",0");
					continue;
				}
		...
		public int getPDV()
		{
			int pdv = _PDV + getBuffValue(Variable.STATS_ADD_VITA);;
			return pdv;
		}
		...
		public void applyBeginningTurnBuff(Fight fight)
		{
			...
			{
				for(int effectID : Variable.BEGIN_TURN_BUFF)
				{
				...
				}else
				{
					...
					switch(entry.getEffectID())
					{
						case 150://Invisibilité
							Packets.GAME_SEND_GA_PACKET_TO_FIGHT(_fight, 7, 150, entry.getCaster().getGUID()+"",getGUID()+",0");
						break;
					}
				}
			}
		...
		public void addBuff(int id,int val,int duration,int turns,boolean debuff,int spellID,String args,Fighter caster)
		{
			...
			switch(id)
			{
				case 6://Renvoie de sort
					Packets.GAME_SEND_FIGHT_GIE_TO_FIGHT(_fight, 7, id, getGUID(), -1, val+"", "10", "", duration, spellID);
				break;
				...
				case 79://Chance éca
					...
					Packets.GAME_SEND_FIGHT_GIE_TO_FIGHT(_fight, 7, id, getGUID(), val, valMax, chance, "", duration, spellID);
				break;
				...
				case 782: //Maximise les effets aléatoire		
					...
					{
						Packets.GAME_SEND_FIGHT_GIE_TO_FIGHT(_fight, 7, id, getGUID(), val, "", "", "", duration, spellID);		
					}else if(valMax.compareTo("-1") != 0)
					{
						Packets.GAME_SEND_FIGHT_GIE_TO_FIGHT(_fight, 7, id, getGUID(), val, valMax, "", "", duration, spellID);
					}
					break;
				...
				case 788://Fait apparaitre message le temps de buff sacri Chatiment de X sur Y tours
					...
					Packets.GAME_SEND_FIGHT_GIE_TO_FIGHT(_fight, 7, id, getGUID(), val, ""+val, ""+valMax2, "", duration, spellID);
					break;
					
				case 950:
					Packets.GAME_SEND_FIGHT_GIE_TO_FIGHT(_fight, 7, id, getGUID(), -1, "", val+"", "", duration, spellID);
					break;
			...
			if(_perso!=null) {
				Packets.GAME_SEND_STATS_PACKET(_perso);
			}
		}
		...
		{
			return _PDVMAX + getBuffValue(Variable.STATS_ADD_VITA);
		}
		...
		public String xpString(String str)
		{
			if(_perso != null)
			{
				...
				if(max>Main.MAX_LEVEL)max = Main.MAX_LEVEL;
		...
		{
			if(_type == 1)
				return getTotalStats().getEffect(Variable.STATS_ADD_PA);
			if(_type == 2)
				return getTotalStats().getEffect(Variable.STATS_ADD_PA) + _mob.getPA();
		...
		{
			if(_type == 1)
				return getTotalStats().getEffect(Variable.STATS_ADD_PM);
			if(_type == 2)
				return getTotalStats().getEffect(Variable.STATS_ADD_PM) + _mob.getPM();
			return 0;
		}
		...
		public void debuff()
		{
			...
			for(SpellEffect SE : _fightBuffs)
			{
				...
				switch(SE.getEffectID())
				{
					...
					case Variable.STATS_ADD_PA2:
						Packets.GAME_SEND_GA_PACKET_TO_FIGHT(_fight, 7, 101,getGUID()+"",getGUID()+",-"+SE.getValue());
					break;
					
					case Variable.STATS_ADD_PM:
					case Variable.STATS_ADD_PM2:
						Packets.GAME_SEND_GA_PACKET_TO_FIGHT(_fight, 7, 127,getGUID()+"",getGUID()+",-"+SE.getValue());
					break;
				}
			}
			_fightBuffs.clear();
		...
		public void unHide()
		{
			...
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(_fight, 7, 150,getGUID()+"",getGUID()+",0");
			//On actualise la position
			Packets.GAME_SEND_GIC_PACKET_TO_FIGHT(_fight, 7,this);
		}
	...
	public static class Glyphe
	{
		...
		public Glyphe(Fight fight, Fighter caster, Case cell, byte size, SortStats trapSpell, byte duration, int spell)
		{
			...
			_trapSpell = trapSpell;
			_duration = duration;
			_color = Variable.getGlyphColor(spell);
		}
		...
		public void desapear()
		{
			Packets.GAME_SEND_GDZ_PACKET_TO_FIGHT(_fight, 7, "-",_cell.getID(), _size, _color);
			Packets.GAME_SEND_GDC_PACKET_TO_FIGHT(_fight, 7, _cell.getID());
		}
	...
	public Fight(int type, int id,Maps map, Player init1, Player init2)
	{
		...
		Packets.GAME_SEND_FIGHT_GJK_PACKET_TO_FIGHT(this,7,2,_type==Variable.FIGHT_TYPE_CHALLENGE?1:0,1,0,_type==Variable.FIGHT_TYPE_CHALLENGE?0:Main.CONFIG_BEGIN_TIME,_type);
		//on desactive le timer de regen coté client
		Packets.GAME_SEND_ILF_PACKET(init1, 0);
		Packets.GAME_SEND_ILF_PACKET(init2, 0);
		
		if(_type!=Variable.FIGHT_TYPE_CHALLENGE)
		{
			_turnTimer = new Timer(Main.CONFIG_BEGIN_TIME,new ActionListener()
			{
		...
		if(teams.nextBoolean())
		{
			...
			Packets.GAME_SEND_FIGHT_PLACES_PACKET_TO_FIGHT(this,1,_map.get_placesStr(),0);
			Packets.GAME_SEND_FIGHT_PLACES_PACKET_TO_FIGHT(this,2,_map.get_placesStr(),1);
			_st1 = 0;
			_st2 = 1;
		}else
		{
			...
			_st1 = 1;
			_st2 = 0;
			Packets.GAME_SEND_FIGHT_PLACES_PACKET_TO_FIGHT(this,1,_map.get_placesStr(),1);
			Packets.GAME_SEND_FIGHT_PLACES_PACKET_TO_FIGHT(this,2,_map.get_placesStr(),0);
		}
		Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, init1.get_GUID()+"", init1.get_GUID()+","+Variable.ETAT_PORTE+",0");
		Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, init1.get_GUID()+"", init1.get_GUID()+","+Variable.ETAT_PORTEUR+",0");
		Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, init2.get_GUID()+"", init2.get_GUID()+","+Variable.ETAT_PORTE+",0");
		Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, init2.get_GUID()+"", init2.get_GUID()+","+Variable.ETAT_PORTEUR+",0");
		...
		_init1.setTeam(1);
		Packets.GAME_SEND_ERASE_ON_MAP_TO_MAP(_init0.getPersonnage().get_curCarte(), _init0.getGUID());
		Packets.GAME_SEND_ERASE_ON_MAP_TO_MAP(_init1.getPersonnage().get_curCarte(), _init1.getGUID());
		
		Packets.GAME_SEND_GAME_ADDFLAG_PACKET_TO_MAP(_init0.getPersonnage().get_curCarte(),0,_init0.getGUID(),_init1.getGUID(),_init0.getPersonnage().get_curCell().getID(),"0;-1", _init1.getPersonnage().get_curCell().getID(), "0;-1");
		Packets.GAME_SEND_ADD_IN_TEAM_PACKET_TO_MAP(_init0.getPersonnage().get_curCarte(),_init0.getGUID(), _init0);
		Packets.GAME_SEND_ADD_IN_TEAM_PACKET_TO_MAP(_init0.getPersonnage().get_curCarte(),_init1.getGUID(), _init1);
		
		Packets.GAME_SEND_MAP_FIGHT_GMS_PACKETS_TO_FIGHT(this,7,_map);
		
		set_state(Variable.FIGHT_STATE_PLACE);
	}
	
	public Fight(int id,Maps map,Player init1, MobGroup group)
	{
		_mobGroup = group;
		_type = Variable.FIGHT_TYPE_PVM; //(1: Défie) 2: Pvm (3:PVP)
		_id = id;
		_map = map.getMapCopy();
		_init0 = new Fighter(this,init1);
		...
		Packets.GAME_SEND_FIGHT_GJK_PACKET_TO_FIGHT(this,1,2,0,1,0,Main.CONFIG_BEGIN_TIME,_type);
		...
		for(Entry<Integer,Fighter> entry : e)
		{
			...
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, f.getGUID()+"", f.getGUID()+","+Variable.ETAT_PORTE+",0");
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, f.getGUID()+"", f.getGUID()+","+Variable.ETAT_PORTEUR+",0");
		...
		Packets.GAME_SEND_ERASE_ON_MAP_TO_MAP(_init0.getPersonnage().get_curCarte(), _init0.getGUID());
		Packets.GAME_SEND_ERASE_ON_MAP_TO_MAP(_init0.getPersonnage().get_curCarte(), group.getID());
		
		Packets.GAME_SEND_GAME_ADDFLAG_PACKET_TO_MAP(_init0.getPersonnage().get_curCarte(),4,_init0.getGUID(),group.getID(),(_init0.getPersonnage().get_curCell().getID()+1),"0;-1",group.getCellID(),"1;-1");
		Packets.GAME_SEND_ADD_IN_TEAM_PACKET_TO_MAP(_init0.getPersonnage().get_curCarte(),_init0.getGUID(), _init0);
		
		for(Fighter f : _team1.values())
		{
			Packets.GAME_SEND_ADD_IN_TEAM_PACKET_TO_MAP(_init0.getPersonnage().get_curCarte(),group.getID(), f);
		}
	...
	{
		return Cipher.parseStartCell(_map, num);
	}
	...
	public synchronized void changePlace(Player perso,int cell)
	{
		...
		Packets.GAME_SEND_FIGHT_CHANGE_PLACE_PACKET_TO_FIGHT(this,3,_map,perso.get_GUID(),cell);
	}
	...
	public void verifIfAllReady()
	{
		...
		if(_type != Variable.FIGHT_TYPE_PVM)
		{
	...
	{
		if(_state >= Variable.FIGHT_STATE_ACTIVE)return;
		_state = Variable.FIGHT_STATE_ACTIVE;
		_startTime = System.currentTimeMillis();
		Packets.GAME_SEND_GAME_REMFLAG_PACKET_TO_MAP(_init0.getPersonnage().get_curCarte(),_init0.getGUID());
		if(_type == Variable.FIGHT_TYPE_PVM)
		{
			int align = -1;
			if(_team1.size() >0)
		...
		Packets.GAME_SEND_GIC_PACKETS_TO_FIGHT(this, 7);
		Packets.GAME_SEND_GS_PACKET_TO_FIGHT(this, 7);
		InitOrdreJeu();
		_curPlayer = -1;
		Packets.GAME_SEND_GTL_PACKET_TO_FIGHT(this,7);
		Packets.GAME_SEND_GTM_PACKET_TO_FIGHT(this, 7);
		if(_turnTimer  != null)_turnTimer.stop();
		_turnTimer = null;
		_turnTimer = new Timer(Main.TIME_BY_TURN,new ActionListener()
			{
				public void actionPerformed(ActionEvent e)
				{
					endTurn();
		...
		for(Fighter F : getFighters(3))
		{
			...
			if(perso.isOnMount())
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, perso.get_GUID()+"", perso.get_GUID()+","+Variable.ETAT_CHEVAUCHANT+",1");
	...
	private void startTurn()
	{
		...
		if(_state >= Variable.FIGHT_STATE_FINISHED)return;
		...
		if(_ordreJeu.get(_curPlayer).getPersonnage() != null)
			Packets.GAME_SEND_STATS_PACKET(_ordreJeu.get(_curPlayer).getPersonnage());
		
		if(_ordreJeu.get(_curPlayer).hasBuff(Variable.EFFECT_PASS_TURN))//Si il doit passer son tour
		{
		...
		Packets.GAME_SEND_GAMETURNSTART_PACKET_TO_FIGHT(this,7,_ordreJeu.get(_curPlayer).getGUID(),Main.TIME_BY_TURN);
		_turnTimer.restart();
		
		if(_ordreJeu.get(_curPlayer).getPersonnage() == null)
        {
           AI.launchIA(_ordreJeu.get(_curPlayer), this);
           endTurn();
        }
	}
	...
	public void endTurn()
	{
		...
		if(_ordreJeu.get(_curPlayer) != null)
			Packets.GAME_SEND_GAMETURNSTOP_PACKET_TO_FIGHT(this,7,_ordreJeu.get(_curPlayer).getGUID());
		...
		try
		{
			...
			if(!_ordreJeu.get(_curPlayer).hasLeft())
			{
				...
				if(!_ordreJeu.get(_curPlayer).isDead())
				{
					...
					for(SpellEffect SE : _ordreJeu.get(_curPlayer).getBuffsByEffectID(131))
					{
						...
						if(SE.getSpell() == 200)
						{
							int inte = SE.getCaster().getTotalStats().getEffect(Variable.STATS_ADD_INTE);
							if(inte < 0)inte = 0;
							int pdom = SE.getCaster().getTotalStats().getEffect(Variable.STATS_ADD_PERDOM);
							if(pdom < 0)pdom = 0;
						...
						dgt = -(dgt);
						Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 100, SE.getCaster().getGUID()+"", _ordreJeu.get(_curPlayer).getGUID()+","+dgt);
				...
				_curFighterUsedPA = 0;
				_curFighterUsedPM = 0;
				_curFighterPA = _ordreJeu.get(_curPlayer).getTotalStats().getEffect(Variable.STATS_ADD_PA);
				_curFighterPM = _ordreJeu.get(_curPlayer).getTotalStats().getEffect(Variable.STATS_ADD_PM);
			...
			Packets.GAME_SEND_GTM_PACKET_TO_FIGHT(this, 7);
			Packets.GAME_SEND_GTR_PACKET_TO_FIGHT(this, 7, _ordreJeu.get(_curPlayer==_ordreJeu.size()?0:_curPlayer).getGUID());
	...
	{	
		...
		if(_team0.containsKey(guid))
		{
			...
			if(onlyGroup0)
			{
				...
				if(g != null)
				{
					...
					{
						Packets.GAME_SEND_GA903_ERROR_PACKET(perso.get_compte().getGameThread().get_out(),'f',guid);
						return;
					}
				}
			}
			if(_type == Variable.FIGHT_TYPE_AGRESSION)
			{
				if(perso.get_align() == Variable.ALIGNEMENT_NEUTRE)
				{
					Packets.GAME_SEND_GA903_ERROR_PACKET(perso.get_compte().getGameThread().get_out(),'f',guid);
					return;
				}
			...
			Packets.GAME_SEND_GJK_PACKET(perso,2,1,1,0,0,_type);
			Packets.GAME_SEND_FIGHT_PLACES_PACKET(perso.get_compte().getGameThread().get_out(), _map.get_placesStr(), _st1);
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, perso.get_GUID()+"", perso.get_GUID()+","+Variable.ETAT_PORTE+",0");
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, perso.get_GUID()+"", perso.get_GUID()+","+Variable.ETAT_PORTEUR+",0");
			Packets.GAME_SEND_ERASE_ON_MAP_TO_MAP(perso.get_curCarte(), perso.get_GUID());
			...
			Packets.GAME_SEND_ILF_PACKET(perso, 0);
		}else if(_team1.containsKey(guid))
		{
			...
			Packets.GAME_SEND_GJK_PACKET(perso,2,1,1,0,0,0);
			Packets.GAME_SEND_FIGHT_PLACES_PACKET(perso.get_compte().getGameThread().get_out(), _map.get_placesStr(), _st2);
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, perso.get_GUID()+"", perso.get_GUID()+","+Variable.ETAT_PORTE+",0");
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, perso.get_GUID()+"", perso.get_GUID()+","+Variable.ETAT_PORTEUR+",0");
			Packets.GAME_SEND_ERASE_ON_MAP_TO_MAP(perso.get_curCarte(), perso.get_GUID());
			Fighter f = new Fighter(this, perso);
		...
		Packets.GAME_SEND_ADD_IN_TEAM_PACKET_TO_MAP(perso.get_curCarte(),(current_Join.getTeam()==0?_init0:_init1).getGUID(), current_Join);
		Packets.GAME_SEND_FIGHT_PLAYER_JOIN(this,7,current_Join);
		Packets.GAME_SEND_MAP_FIGHT_GMS_PACKETS(this,_map,perso);
	}
	...
	public void toggleLockTeam(int guid)
	{
		if(_init0.getGUID() == guid)
		{
			...
			Packets.GAME_SEND_FIGHT_CHANGE_OPTION_PACKET_TO_MAP(_init0.getPersonnage().get_curCarte(), locked0?'+':'-', 'A', guid);
			Packets.GAME_SEND_Im_PACKET_TO_FIGHT(this,1,locked0?"095":"096");
		}else if(_init1.getGUID() == guid)
		{
			locked1 = !locked1;
			GameServer.addToLog(locked1?"L'équipe 2 devient bloquée":"L'équipe 2 n'est plus bloquée");
			Packets.GAME_SEND_FIGHT_CHANGE_OPTION_PACKET_TO_MAP(_init1.getPersonnage().get_curCarte(), locked1?'+':'-', 'A', guid);
			Packets.GAME_SEND_Im_PACKET_TO_FIGHT(this,2,locked1?"095":"096");
		}
	}
	...
	public void toggleOnlyGroup(int guid)
	{
		if(_init0.getGUID() == guid)
		{
			...
			Packets.GAME_SEND_FIGHT_CHANGE_OPTION_PACKET_TO_MAP(_init0.getPersonnage().get_curCarte(), onlyGroup0?'+':'-', 'P', guid);
			Packets.GAME_SEND_Im_PACKET_TO_FIGHT(this,1,onlyGroup0?"093":"094");
		}else if(_init1.getGUID() == guid)
		{
			...
			Packets.GAME_SEND_FIGHT_CHANGE_OPTION_PACKET_TO_MAP(_init1.getPersonnage().get_curCarte(), onlyGroup1?'+':'-', 'P', guid);
			Packets.GAME_SEND_Im_PACKET_TO_FIGHT(this,2,onlyGroup1?"095":"096");
		}
	}
	...
	public void toggleLockSpec(int guid)
	{
		if(_init0.getGUID() == guid || _init1.getGUID() == guid)
		{
			...
			Packets.GAME_SEND_FIGHT_CHANGE_OPTION_PACKET_TO_MAP(_init0.getPersonnage().get_curCarte(), specOk?'+':'-', 'S', _init0.getGUID());
			Packets.GAME_SEND_FIGHT_CHANGE_OPTION_PACKET_TO_MAP(_init0.getPersonnage().get_curCarte(), specOk?'+':'-', 'S', _init1.getGUID());
			Packets.GAME_SEND_Im_PACKET_TO_MAP(_map,specOk?"039":"040");
		}
	}
	...
	public void toggleHelp(int guid)
	{
		if(_init0.getGUID() == guid)
		{
			...
			Packets.GAME_SEND_FIGHT_CHANGE_OPTION_PACKET_TO_MAP(_init0.getPersonnage().get_curCarte(), locked0?'+':'-', 'H', guid);
			Packets.GAME_SEND_Im_PACKET_TO_FIGHT(this,1,help1?"0103":"0104");
		}else if(_init1.getGUID() == guid)
		{
			help2 = !help2;
			GameServer.addToLog(help2?"L'équipe 2 demande de l'aide":"L'équipe 2 ne demande plus d'aide");
			Packets.GAME_SEND_FIGHT_CHANGE_OPTION_PACKET_TO_MAP(_init1.getPersonnage().get_curCarte(), locked1?'+':'-', 'H', guid);
			Packets.GAME_SEND_Im_PACKET_TO_FIGHT(this,2,help2?"0103":"0104");
		}
	}
	...
	public boolean fighterDeplace(Fighter f, GameAction GA)
	{
		...
		if(!_curAction.equals("")|| _ordreJeu.get(_curPlayer).getGUID() != f.getGUID() || _state!= Variable.FIGHT_STATE_ACTIVE)
		{
			...
			if(_state != Variable.FIGHT_STATE_ACTIVE)
		...
		if( tacle != null)//Tentative de Tacle
		{
			...
			int chance = Utils.getTacleChance(f, tacle);
			int rand = Utils.getRandomValue(0, 99);
			if(rand > chance)
			{
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7,GA._id, "104",_ordreJeu.get(_curPlayer).getGUID()+";", "");//Joueur taclé
				int pertePA = _curFighterPA*chance/100;
				
				if(pertePA  < 0)pertePA = -pertePA;
				if(_curFighterPM < 0)_curFighterPM = -_curFighterPM;
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7,GA._id,"129", f.getGUID()+"", f.getGUID()+",-"+_curFighterPM);
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7,GA._id,"102", f.getGUID()+"", f.getGUID()+",-"+pertePA);
		...
		int nextCellID = Cipher.cellCode_To_ID(newPath.substring(newPath.length() - 2));
		//les monstres n'ont pas de GAS//GAF
		if(_ordreJeu.get(_curPlayer).getPersonnage() != null)
			Packets.GAME_SEND_GAS_PACKET_TO_FIGHT(this,7,_ordreJeu.get(_curPlayer).getGUID());
        //Si le joueur n'est pas invisible
        if(!_ordreJeu.get(_curPlayer).isHide())
	        Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, GA._id, "1", _ordreJeu.get(_curPlayer).getGUID()+"", "a"+Cipher.cellID_To_Code(f.get_fightCell().getID())+newPath);
        else//Si le joueur est planqué x)
        {
        	if(_ordreJeu.get(_curPlayer).getPersonnage() != null)
        	{
        		...
        		Packets.GAME_SEND_GA_PACKET(out,  GA._id+"", "1", _ordreJeu.get(_curPlayer).getGUID()+"", "a"+Cipher.cellID_To_Code(f.get_fightCell().getID())+newPath);
        	}
        }
        ...
        if(po != null
        && _ordreJeu.get(_curPlayer).isState(Variable.ETAT_PORTE)
        && po.isState(Variable.ETAT_PORTEUR))
        {
        	...
        	//si le joueur va bouger
       		...
       		{
       			//on retire les états
       			po.setState(Variable.ETAT_PORTEUR, 0);
       			_ordreJeu.get(_curPlayer).setState(Variable.ETAT_PORTE,0);
       			...
       			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 950, po.getGUID()+"", po.getGUID()+","+Variable.ETAT_PORTEUR+",0");
    			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 950, _ordreJeu.get(_curPlayer).getGUID()+"", _ordreJeu.get(_curPlayer).getGUID()+","+Variable.ETAT_PORTE+",0");
       		}
      	}
        
		_ordreJeu.get(_curPlayer).get_fightCell().getFighters().clear();
        GameServer.addToLog("("+_curPlayer+") Fighter ID= "+f.getGUID()+" se déplace de la case "+_ordreJeu.get(_curPlayer).get_fightCell().getID()+" vers "+Cipher.cellCode_To_ID(newPath.substring(newPath.length() - 2)));
        ...
        if(po != null
        && _ordreJeu.get(_curPlayer).isState(Variable.ETAT_PORTEUR)
        && po.isState(Variable.ETAT_PORTE))
        {
        ...
        if(f.getPersonnage() == null)
        {
        	...
        	Packets.GAME_SEND_GAMEACTION_TO_FIGHT(this,7,_curAction);
    		_curAction = "";
	...
	{
		if(_curAction.equals("")|| _ordreJeu.get(_curPlayer).getGUID() != perso.get_GUID() || _state!= Variable.FIGHT_STATE_ACTIVE)return;
		GameServer.addToLog("("+_curPlayer+")Fin du déplacement de Fighter ID= "+perso.get_GUID());
		Packets.GAME_SEND_GAMEACTION_TO_FIGHT(this,7,_curAction);
		Packets.GAME_SEND_GAF_PACKET_TO_FIGHT(this,7,2,_ordreJeu.get(_curPlayer).getGUID());
		//copie
		ArrayList<Piege> P = (new ArrayList<Piege>());
		P.addAll(_traps);
		for(Piege p : P)
		{
			...
			//on active le piege
			if(dist <= p.get_size())p.onTraped(F);
			if(_state == Variable.FIGHT_STATE_FINISHED)break;
		}
		try {
			Thread.sleep(500);
		} catch (InterruptedException e) {};
	...
	public int tryCastSpell(Fighter fighter,SortStats Spell, int caseID)
	{
		...
		if(CanCastSpell(fighter,Spell,Cell))
		{
			if(fighter.getPersonnage() != null)
				Packets.GAME_SEND_STATS_PACKET(fighter.getPersonnage());
			...
			Packets.GAME_SEND_GAS_PACKET_TO_FIGHT(this, 7, fighter.getGUID());
			boolean isEc = Spell.getTauxEC() != 0 && Utils.getRandomValue(1, Spell.getTauxEC()) == Spell.getTauxEC();
			if(isEc)
			{
				GameServer.addToLog(fighter.getPacketsName()+" Echec critique sur le sort "+Spell.getSpellID());
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 302, fighter.getGUID()+"", Spell.getSpellID()+"");
			}else
			{
				...
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 300, fighter.getGUID()+"", sort);	
				if(isCC)
				{
					GameServer.addToLog(fighter.getPacketsName()+" Coup critique sur le sort "+Spell.getSpellID());
					Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 301, fighter.getGUID()+"", sort);
				}
			...
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 102,fighter.getGUID()+"",fighter.getGUID()+",-"+Spell.getPACost());
			Packets.GAME_SEND_GAF_PACKET_TO_FIGHT(this, 7, 0, fighter.getGUID());
			//Refresh des Stats
	...
	public boolean CanCastSpell(Fighter fighter, SortStats spell, Case cell)
	{
		...
		if(spell.isModifPO())
			MaxPO += fighter.getTotalStats().getEffect(Variable.STATS_ADD_PO);
	...
	public String GetGE(int win)
    {
		...
		int type = Variable.FIGHT_TYPE_CHALLENGE;// toujours 0
		if(_type == Variable.FIGHT_TYPE_AGRESSION)//Sauf si gain d'honneur
			type = _type;
        ...
        //DROP SYSTEM
	        ...
	        for(Fighter F : TEAM1)
        	{
	        	if(!F.isInvocation() || (F.getMob() != null && F.getMob().getTemplate().getID() ==258))
	        		groupPP += F.getTotalStats().getEffect(Variable.STATS_ADD_PROS);
        	}
	        ...
	        for(Fighter F : TEAM2)
	        {
	        	...
	        	for(Drop D : F.getMob().getDrops())
	        	{
	        		if(D.getMinProsp() <= groupPP)
	        		{
	        			...
	        			int taux = (int)((groupPP * D.get_taux()*Main.CONFIG_DROP)/100);
	        ...
	        while(Temp.size() < TEAM1.size())
	        {
		        ...
		        for(Fighter F : TEAM1)
		        {
		        	...
		        	if(F.getTotalStats().getEffect(Variable.STATS_ADD_PROS) > curPP && !Temp.contains(F))
		        	{
		        		curMax = F;
		        		curPP = F.getTotalStats().getEffect(Variable.STATS_ADD_PROS);
		        	}
		        }
	        	Temp.add(curMax);
	        }
	    ...
	    //Capture d'âmes
	        ...
	        if(isCapturable)
	        {
		        ...
		        pierrePleine = new SoulStone(World.getNewItemGuid(),1,7010,Variable.ITEM_POS_NO_EQUIPED,pierreStats);	//Crée la pierre d'âme
		        
		        for(Fighter F : TEAM1)	//Récupère les captureur
		        {
		        	if(!F.isInvocation() && F.isState(Variable.ETAT_CAPT_AME))
		        		_captureur.add(F);
		        }
		        ...
		        if(_captureur.size() > 0 && !World.isArenaMap(get_map().get_id()))	//S'il y a des captureurs
	    		{
	    			for (int i = 0; i < _captureur.size(); i++)
	    			{
	    				try
	    				{
			        		Fighter f = _captureur.get(Utils.getRandomValue(0, _captureur.size()-1));	//Récupère un captureur au hasard dans la liste
			    			if(!(f.getPersonnage().getObjetByPos(Variable.ITEM_POS_ARME).getTemplate().getType() == Variable.ITEM_TYPE_PIERRE_AME	//Avoir une pierre d'ame vide d'équipé
			    				))
		    				{
			    				_captureur.remove(f);
		    					continue;
		    				}
			    			Couple<Integer,Integer> pierreJoueur = Utils.decompPierreAme(f.getPersonnage().getObjetByPos(Variable.ITEM_POS_ARME));//Récupère les stats de la pierre équippé
			    			...
			    			int captChance = Utils.totalCaptChance(pierreJoueur.first, f.getPersonnage());
			    			
			    			if(Utils.getRandomValue(1, 100) <= captChance)	//Si le joueur obtiens la capture
			    			{
			    				//Retire la pierre vide au personnage et lui envoie ce changement
			    				int pierreVide = f.getPersonnage().getObjetByPos(Variable.ITEM_POS_ARME).getGuid();
			    				f.getPersonnage().deleteItem(pierreVide);
			    				Packets.GAME_SEND_REMOVE_ITEM_PACKET(f.getPersonnage(), pierreVide);
	    ...
	    for(Fighter i : TEAM1)
		{
        	if(type == Variable.FIGHT_TYPE_CHALLENGE)
        	{
        		if(i.isInvocation() && i.getMob() != null && i.getMob().getTemplate().getID() != 258)continue;
        		long winxp 	= Utils.getXpWinPvm2(i,TEAM1,TEAM2,totalXP);
        		AtomicReference<Long> XP = new AtomicReference<Long>();
        		XP.set(winxp);
        		
        		long guildxp = Utils.getGuildXpWin(i,XP);
        		long mountxp = 0;
        		if(i.getPersonnage() != null && i.getPersonnage().isOnMount())
        		{
        			mountxp = Utils.getMountXpWin(i,XP);
        			i.getPersonnage().getMount().addXp(mountxp);
        			Packets.GAME_SEND_Re_PACKET(i.getPersonnage(),"+",i.getPersonnage().getMount());
        		}
        		int winKamas	= Utils.getKamasWin(i,TEAM1,minkamas,maxkamas);
        		String drops = "";
        		//Drop system
        		...
        		for(Drop D : temp)
        		{
        			...
        			int jet = Utils.getRandomValue(0, 100*100);
        			if(jet < t)
        			{
        	...
        	}else
        	{
    			...
    			{
        			if(_init1.getPersonnage().get_compte().get_curIP().compareTo(_init0.getPersonnage().get_compte().get_curIP()) != 0)
            		{
            			winH = Utils.calculHonorWin(TEAM1,TEAM2,i);
        			}
    			}
        		...
        		Packet += "2;" + i.getGUID() + ";" + i.getPacketsName() + ";" + i.get_lvl() + ";" + (i.isDead() ?  "1" : "0" )+";";
        		Packet += (P.get_align()!=Variable.ALIGNEMENT_NEUTRE?World.getExpLevel(P.getGrade()).pvp:0)+";";
        		Packet += P.get_honor()+";";
        		int maxHonor = World.getExpLevel(P.getGrade()+1).pvp;
        		if(maxHonor == -1)maxHonor = World.getExpLevel(P.getGrade()).pvp;
        		Packet += (P.get_align()!=Variable.ALIGNEMENT_NEUTRE?maxHonor:0)+";";
        		Packet += winH+";";
        		Packet += P.getGrade()+";";
		...
		for(Fighter i : TEAM2)
		{
        	...
        	{
        		//Calcul honeur
        		int winH = Utils.calculHonorWin(TEAM1,TEAM2,i);
        		int winD = Utils.calculDeshonorWin(TEAM1,TEAM2,i);
	...
	public void verifIfTeamAllDead()
	{
		...
		{
			_state = Variable.FIGHT_STATE_FINISHED;
			...
			for(Entry<Integer, Fighter> entry : _team0.entrySet())
			{
				Packets.GAME_SEND_ERASE_ON_MAP_TO_MAP(_map, entry.getValue().getGUID());
			}
			for(Entry<Integer, Fighter> entry : _team1.entrySet())
			{
				Packets.GAME_SEND_ERASE_ON_MAP_TO_MAP(_map, entry.getValue().getGUID());
			...
			Packets.GAME_SEND_FIGHT_GE_PACKET_TO_FIGHT(this,7,winner);
			...
			for(Entry<Integer, Fighter> entry : _team0.entrySet())
			{
				...
				if(_type==Variable.FIGHT_TYPE_CHALLENGE)perso.fullPDV();
			}
			switch(_type)
			{
				
				case Variable.FIGHT_TYPE_CHALLENGE://Défie
				for(Entry<Integer, Fighter> entry : _team1.entrySet())
				{
					Player perso = entry.getValue().getPersonnage();
					if(perso == null)continue;
				...
				break;
				case Variable.FIGHT_TYPE_AGRESSION://Aggro
					for(Entry<Integer, Fighter> entry : _team1.entrySet())
					{
						Player perso = entry.getValue().getPersonnage();
						if(perso == null)continue;
				...
				break;
				case Variable.FIGHT_TYPE_PVM://PvM
			...
			Packets.GAME_SEND_MAP_FIGHT_COUNT_TO_MAP(World.getCarte(_map.get_id()));
			_map = null;
			_ordreJeu = null;
	...
	public void onFighterDie(Fighter target) 
	{
		...
		Packets.GAME_SEND_FIGHT_PLAYER_DIE_TO_FIGHT(this,7,target.getGUID());
		...
		if(target.getTeam() == 0)
		{
			...
			for(Entry<Integer,Fighter> entry : team.entrySet())
			{
				...
				if(entry.getValue().getInvocator().getGUID() == target.getGUID())//si il a été invoqué par le joueur mort
				{
					...
					Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 999, target.getGUID()+"", getGTL());
				}
			}
		...
		if(target.getMob() != null)
		{
			...
			try
			{
				...
				for(int id : Variable.STATIC_INVOCATIONS)if(id == target.getMob().getTemplate().getID())isStatic = true;
		...
		for(Glyphe g : glyphs)
		{
			...
			{
				Packets.GAME_SEND_GDZ_PACKET_TO_FIGHT(this, 7, "-", g.get_cell().getID(), g.get_size(), 4);
				Packets.GAME_SEND_GDC_PACKET_TO_FIGHT(this, 7, g.get_cell().getID());
				_glyphs.remove(g);
			}
		}
	...
	public void tryCaC(Player perso, int cellID)
	{
		...
		if(perso.getObjetByPos(Variable.ITEM_POS_ARME) == null)//S'il n'a pas de CaC
		{
			if(_curFighterPA < 4)//S'il n'a pas assez de PA
				return;
			Packets.GAME_SEND_GAS_PACKET_TO_FIGHT(this, 7, perso.get_GUID());
			//Si le joueur est invisible
			if(caster.isHide())caster.unHide();
			
			Fighter target = _map.getCase(cellID).getFirstFighter();
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 303, perso.get_GUID()+"", cellID+"");
			
			if(target != null)
			{
				int dmg = Utils.getRandomJet("1d5+0");
				int finalDommage = Utils.calculFinalDommage(this,caster, target,Variable.ELEMENT_NEUTRE, dmg,false);
				...
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 100, caster.getGUID()+"", target.getGUID()+","+finalDommage);
			}
			_curFighterPA-= 4;
			Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 102,perso.get_GUID()+"",perso.get_GUID()+",-4");
			Packets.GAME_SEND_GAF_PACKET_TO_FIGHT(this, 7, 0, perso.get_GUID());
		...
		{
			Item arme = perso.getObjetByPos(Variable.ITEM_POS_ARME);
			...
			Packets.GAME_SEND_GAS_PACKET_TO_FIGHT(this, 7, perso.get_GUID());
			boolean isEc = arme.getTemplate().getTauxEC() != 0 && Utils.getRandomValue(1, arme.getTemplate().getTauxEC()) == arme.getTemplate().getTauxEC();
			if(isEc)
			{
				GameServer.addToLog(perso.get_name()+" Echec critique sur le CaC ");
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 305, perso.get_GUID()+"", "");//Echec Critique Cac
				Packets.GAME_SEND_GAF_PACKET_TO_FIGHT(this, 7, 0, perso.get_GUID());//Fin de l'action
				endTurn();
			}else
			{
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 303, perso.get_GUID()+"", cellID+"");
				...
				if(isCC)
				{
					...
					Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 301, perso.get_GUID()+"", "0");
				}
				...
				{
					if(_state != Variable.FIGHT_STATE_ACTIVE)break;
				...
				Packets.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 102,perso.get_GUID()+"",perso.get_GUID()+",-"+PACost);
				Packets.GAME_SEND_GAF_PACKET_TO_FIGHT(this, 7, 0, perso.get_GUID());
				verifIfTeamAllDead();
			}
		}
	...
	{
		_curFighterPA = _ordreJeu.get(_curPlayer).getTotalStats().getEffect(Variable.STATS_ADD_PA) - _curFighterUsedPA;
		_curFighterPM = _ordreJeu.get(_curPlayer).getTotalStats().getEffect(Variable.STATS_ADD_PM) - _curFighterUsedPM;
	}

	public void leftFight(Player perso)
	{
		...
		if(F != null)
		{
			if(_state >= Variable.FIGHT_STATE_ACTIVE)
			{
				...
				if(_state == Variable.FIGHT_STATE_ACTIVE)
				{
					Packets.GAME_SEND_ERASE_ON_MAP_TO_MAP(_map, F.getGUID());
					if(perso.isOnline())
					{
						/*
						...
						int win = -1;
						//*/
						Packets.GAME_SEND_GV_PACKET(perso);
	...
	public String parseFightInfos()
	{
		...
		switch(_type)
		{
			case Variable.FIGHT_TYPE_CHALLENGE:
				infos += "0,";
				infos += _init1.getPersonnage().get_align()+",";
				infos += _team1.size()+";";
			break;
			
			case Variable.FIGHT_TYPE_AGRESSION:
				infos += "0,";
				infos += _init1.getPersonnage().get_align()+",";
				infos += _team1.size()+";";
			break;
			
			case Variable.FIGHT_TYPE_PVM:
	...
	public void showCaseToTeam(int guid, int cellID)
	{
		...
		Packets.GAME_SEND_FIGHT_SHOW_CASE(PWs, guid, cellID);
	}
	...
	{
		if(!specOk  || _state != Variable.FIGHT_STATE_ACTIVE)
		{
			Packets.GAME_SEND_Im_PACKET(p, "157");
			return;
		}
		p.get_curCell().removePlayer(p.get_GUID());
		Packets.GAME_SEND_GJK_PACKET(p, _state, 0, 0, 1, 0, _type);
		Packets.GAME_SEND_GS_PACKET(p);
		Packets.GAME_SEND_GTL_PACKET(p,this);
		Packets.GAME_SEND_ERASE_ON_MAP_TO_MAP(p.get_curCarte(), p.get_GUID());
		Packets.GAME_SEND_MAP_FIGHT_GMS_PACKETS(this,_map,p);
		Packets.GAME_SEND_GAMETURNSTART_PACKET(p,_ordreJeu.get(_curPlayer).getGUID(),Main.TIME_BY_TURN);
		_spec.put(p.get_GUID(), p);
		p.set_fight(this);
		Packets.GAME_SEND_Im_PACKET_TO_FIGHT(this, 7, "036;"+p.get_name());
	}
