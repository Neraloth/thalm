...
import org.thalm.game.character.Command;
...
import org.thalm.game.character.item.ActionHouse;
...
import org.thalm.game.fight.Fight.Fighter;
...
public class GameClient implements Runnable
{
	...
	public void run()
	{
		try
    	{
			...
			Packets.GAME_SEND_HELLOGAME_PACKET(_out);
	    	while(_in.read(charCur, 0, 1)!=-1 && Main.isRunning)
	    	{
		    	...
		    	{
		    		packet = Cipher.toUnicode(packet);	//MARTHIEUBEAN
		    		
		    		if(_perso != null && Main.canLog)
	...
	private void guild_changeRight(String packet)
	{
		...
		Player p = World.getPersonnage(guid);	//Cherche le personnage a qui l'on change les droits dans la mémoire
		...
		{
			int guildId = Database.isPersoInGuild(guid);	//Récupère l'id de la guilde du personnage qui n'est pas dans la mémoire
			...
			{
				Packets.GAME_SEND_gK_PACKET(_perso, "Ed");
				return;
			}
		...
		else
		{
			...
			{
				Packets.GAME_SEND_gK_PACKET(_perso, "Ea");
				return;
			}
		...
		else	//Sinon, c'est un membre normal
		{
			...
			{
				if(!changer.canDo(Variable.G_RANK) || rank == 1)	//S'il ne peut changer les rang ou qu'il veut mettre meneur
					rank = -1; 	//"Reset" le rang
				
				if(!changer.canDo(Variable.G_RIGHT) || right == 1)	//S'il ne peut changer les droits ou qu'il veut mettre les droits de meneur
					right = -1;	//"Reset" les droits
				
				if(!changer.canDo(Variable.G_HISXP) && !changer.canDo(Variable.G_ALLXP) && changer.getGuid() == toChange.getGuid())	//S'il ne peut changer l'XP de personne et qu'il est la cible
					xpGive = -1; //"Reset" l'XP
			}
			
			if(!changer.canDo(Variable.G_ALLXP) && !changer.equals(toChange))	//S'il n'a pas le droit de changer l'XP des autres et qu'il n'est pas la cible
				xpGive = -1; //"Reset" L'XP
		}

		toChange.setAllRights(rank,xpGive,right);
		
		Packets.GAME_SEND_gS_PACKET(_perso,_perso.getGuildMember());
		
		if(p != null && p.get_GUID() != _perso.get_GUID())
			Packets.GAME_SEND_gS_PACKET(p,p.getGuildMember());
	}

	private void guild_CancelCreate()
	{
		Packets.GAME_SEND_gV_PACKET(_perso);
	}
	...
	private void guild_kick(String name)
	{
		...
		if(P == null)
		{
			int infos[] = Database.isPersoInGuild(name);
			guid = infos[0];
			guildId = infos[1];
		...
		if(!_perso.getGuildMember().canDo(Variable.G_BAN) && _perso.getGuildMember().getGuid() != toRemMember.getGuid())
		{
			Packets.GAME_SEND_gK_PACKET(_perso, "Ed");
			return;
		}
		...
		if(_perso.getGuildMember().getGuid() != toRemMember.getGuid())
		{
			...
			Packets.GAME_SEND_gK_PACKET(_perso, "K"+_perso.get_name()+"|"+name);
			if(P != null)
				Packets.GAME_SEND_gK_PACKET(P, "K"+_perso.get_name());
		}else//si quitter
		{
			...
			Packets.GAME_SEND_gK_PACKET(_perso, "K"+name+"|"+name);
		}
	}
	...
	private void guild_join(String packet)
	{
		...
		case 'R'://Nom perso
			...
			{
				Packets.GAME_SEND_gJ_PACKET(_perso, "Eu");
				return;
			}
			if(!P.isOnline())
			{
			...
			if(P.is_away())
			{
				Packets.GAME_SEND_gJ_PACKET(_perso, "Eo");
				return;
			}
			if(P.get_guild() != null)
			{
				Packets.GAME_SEND_gJ_PACKET(_perso, "Ea");
				return;
			}
			if(!_perso.getGuildMember().canDo(Variable.G_INVITE))
			{
				Packets.GAME_SEND_gJ_PACKET(_perso, "Ed");
				return;
			}
			...
			Packets.GAME_SEND_gJ_PACKET(_perso,"R"+packet.substring(1));
			Packets.GAME_SEND_gJ_PACKET(P,"r"+_perso.get_GUID()+"|"+_perso.get_name()+"|"+_perso.get_guild().get_name());
		break;
		case 'E'://ou Refus
			if(packet.substring(1).equalsIgnoreCase(_perso.getInvitation()+""))
			{
				Player p = World.getPersonnage(_perso.getInvitation());
				if(p == null)return;//Pas censé arriver
				Packets.GAME_SEND_gJ_PACKET(p,"Ec");
			}
		break;
		case 'K'://Accepte
			if(packet.substring(1).equalsIgnoreCase(_perso.getInvitation()+""))
			{
				...
				Database.UPDATE_GUILDMEMBER(GM);
				_perso.setGuildMember(GM);
				_perso.setInvitation(-1);
				p.setInvitation(-1);
				//Packet
				Packets.GAME_SEND_gJ_PACKET(p,"Ka"+_perso.get_name());
				Packets.GAME_SEND_gS_PACKET(_perso, GM);
				Packets.GAME_SEND_gJ_PACKET(_perso,"Kj");
			}
		break;
		}
	}
	...
	private void guild_infos(char c)
	{
		...
		case 'B'://Personalisation (Boost)
			Packets.GAME_SEND_gIB_PACKET(_perso);
		break;
		case 'G'://General
			Packets.GAME_SEND_gIG_PACKET(_perso, _perso.get_guild());
		break;
		case 'M'://Members
			Packets.GAME_SEND_gIM_PACKET(_perso, _perso.get_guild(),'+');
		break;
		}
	}
	...
	private void guild_create(String packet)
	{
		...
		{
			Packets.GAME_SEND_gC_PACKET(_perso, "Ea");
			return;
		}
		...
		try
		{
			...
			{
				Packets.GAME_SEND_gC_PACKET(_perso, "Ean");
				return;
			}
			...
			{
				Packets.GAME_SEND_gC_PACKET(_perso, "Eae");
				return;
			}
			if(_perso.get_curCarte().get_id() == 2196)//Temple de création de guilde
			{
				...
				{
					Packets.GAME_SEND_Im_PACKET(_perso, "14");
					return;
				}
			...
			//Packets
			Packets.GAME_SEND_gS_PACKET(_perso, gm);
			Packets.GAME_SEND_gC_PACKET(_perso,"K");
			Packets.GAME_SEND_gV_PACKET(_perso);
	...
	private void Chanels_change(String packet)
	{
		...
		Database.SAVE_PERSONNAGE(_perso, false);
	}
	...
	private void Mount_changeXpGive(String packet)
	{
		try
		{
			...
			Packets.GAME_SEND_Rx_PACKET(_perso);
		}catch(Exception e){};
	}
	...
	private void Mount_name(String name)
	{
		...
		Packets.GAME_SEND_Rn_PACKET(_perso, name);
	}
	...
	private void Mount_ride()
	{
		...
		{
			Packets.GAME_SEND_Re_PACKET(_perso,"Er", null);
			return;
		}
	...
	private void Mount_description(String packet)
	{
		...
		if(DD == null)return;
		Packets.GAME_SEND_MOUNT_DESCRIPTION_PACKET(_perso,DD);
	}
	...
	private void parse_friendPacket(String packet)
	{
		switch(packet.charAt(1))
		{
			...
			break;
			case 'L'://Liste
				Packets.GAME_SEND_FRIENDLIST_PACKET(_perso);
			break;	
		}
	}
	...
	private void Friend_delete(String packet) {
		...
		switch(packet.charAt(2))
		{
			case '%'://nom de perso
				...
				{
					Packets.GAME_SEND_FD_PACKET(_perso, "Ef");
					return;
				}
				guid = P.getAccID();
	...
	private void Friend_add(String packet)
	{
		...
		switch(packet.charAt(2))
		{
			case '%'://nom de perso
				...
				{
					Packets.GAME_SEND_FA_PACKET(_perso, "Ef");
					return;
				}
				guid = P.getAccID();
			break;
	...
	private void group_quit(String packet)
	{
		...
		{
			 g.leave(_perso);
			 Packets.GAME_SEND_PV_PACKET(_out,"");
		}else if(g.isChief(_perso.get_GUID()))//Sinon, c'est qu'il kick un joueur du groupe
		{
			...
			g.leave(t);
			Packets.GAME_SEND_PV_PACKET(t.get_compte().getGameThread().get_out(),""+_perso.get_GUID());
		}
	}
	...
	private void group_invite(String packet)
	{
		...
		if(!target.isOnline())
		{
			Packets.GAME_SEND_GROUP_INVITATION_ERROR(_out,"n"+name);	//Erreur : X n'est pas connecté
			return;
		}
		if(target.getGroup() != null)
		{
			Packets.GAME_SEND_GROUP_INVITATION_ERROR(_out, "a"+name);	//Erreur : X fait déjà parti d'un groupe
			return;
		}
		if(_perso.getGroup() != null && _perso.getGroup().getPersosNumber() == 8)
		{
			Packets.GAME_SEND_GROUP_INVITATION_ERROR(_out, "f");	//Erreur : Le groupe est plein
			return;
		}
		target.setInvitation(_perso.get_GUID());	
		_perso.setInvitation(target.get_GUID());
		Packets.GAME_SEND_GROUP_INVITATION(_out,_perso.get_name(),name);
		Packets.GAME_SEND_GROUP_INVITATION(target.get_compte().getGameThread().get_out(),_perso.get_name(),name);
	}
	...
	private void group_refuse()
	{
		...
		Player t = World.getPersonnage(_perso.getInvitation());
		_perso.setInvitation(0);
		t.setInvitation(0);
		Packets.GAME_SEND_BN(_out);
		Packets.GAME_SEND_PR_PACKET(t);
	}
	...
	private void group_accept(String packet)
	{
		...
		if(g == null)
		{
			...
			Packets.GAME_SEND_GROUP_CREATE(_out,g);
			Packets.GAME_SEND_PL_PACKET(_out,g);
			Packets.GAME_SEND_GROUP_CREATE(t.get_compte().getGameThread().get_out(),g);
			Packets.GAME_SEND_PL_PACKET(t.get_compte().getGameThread().get_out(),g);
			t.setGroup(g);
			Packets.GAME_SEND_ALL_PM_ADD_PACKET(t.get_compte().getGameThread().get_out(),g);
		}
		else
		{
			Packets.GAME_SEND_GROUP_CREATE(_out,g);
			Packets.GAME_SEND_PL_PACKET(_out,g);
			Packets.GAME_SEND_PM_ADD_PACKET_TO_GROUP(g, _perso);
			g.addPerso(_perso);
		}
		_perso.setGroup(g);
		Packets.GAME_SEND_ALL_PM_ADD_PACKET(_out,g);
		Packets.GAME_SEND_PR_PACKET(t);
	}
	...
	private void Object_drop(String packet)
	{
		...
		int dropCell = Utils.getCellIdNotUsed(_perso);
		if(dropCell == 0) {
			Packets.GAME_SEND_Im_PACKET(_perso, "1145");
			return;
		}
		Case cellDrop = _perso.get_curCarte().getCase(dropCell);
		
		if(qua >= obj.getQuantity())
		{
			_perso.removeItem(guid);
			cellDrop.addDroppedItem(obj);
			obj.setPosition(Variable.ITEM_POS_NO_EQUIPED);
			Packets.GAME_SEND_REMOVE_ITEM_PACKET(_perso, guid);
		}else
		{
			obj.setQuantity(obj.getQuantity() - qua);
			Item obj2 = Item.getCloneObjet(obj, qua);
			obj2.setPosition(Variable.ITEM_POS_NO_EQUIPED);
			cellDrop.addDroppedItem(obj2);
			Packets.GAME_SEND_OBJECT_QUANTITY_PACKET(_perso, obj);
		}
		
		Packets.GAME_SEND_Ow_PACKET(_perso);
		Packets.GAME_SEND_GDO_PACKET_TO_MAP(_perso.get_curCarte(),'+',cellDrop.getID(),obj.getTemplate().getID(),0);
	...
	private void Object_use(String packet)
	{
		...
		if(!obj.getTemplate().getConditions().equalsIgnoreCase("") && !Condition.validConditions(_perso,obj.getTemplate().getConditions()))
		{
			Packets.GAME_SEND_Im_PACKET(_perso, "119|43");
			return;
		}
		T.applyAction(_perso,guid);
		if(T.getType() == Variable.ITEM_TYPE_PAIN || T.getType() == Variable.ITEM_TYPE_VIANDE_COMESTIBLE)
            Packets.GAME_SEND_eUK_PACKET_TO_MAP(_perso.get_curCarte(), _perso.get_GUID(), 17);
		else if(T.getType() == Variable.ITEM_TYPE_BIERE)
            Packets.GAME_SEND_eUK_PACKET_TO_MAP(_perso.get_curCarte(), _perso.get_GUID(), 18);
	}
	...
	private synchronized void Object_move(String packet)
	{
		...
		try
		{
			...
			if(!_perso.hasItemGuid(guid) || obj == null)
				return;
			if(_perso.get_fight() != null)return;
			if(!Variable.isValidPlaceForItem(obj.getTemplate(),pos) && pos != Variable.ITEM_POS_NO_EQUIPED)
				return;
			...
			if(exObj != null)//S'il y avait déja un objet => Ne devrait pas arriver, le client envoie déséquiper avant
			{
				...
				if(( obj2 = _perso.getSimilarItem(exObj)) != null)
				{
					...
					Packets.GAME_SEND_OBJECT_QUANTITY_PACKET(_perso, obj2);
					World.removeItem(exObj.getGuid());
					_perso.removeItem(exObj.getGuid());
					Packets.GAME_SEND_REMOVE_ITEM_PACKET(_perso, exObj.getGuid());
				}
				else
				{
					exObj.setPosition(Variable.ITEM_POS_NO_EQUIPED);
					Packets.GAME_SEND_OBJET_MOVE_PACKET(_perso,exObj);
				}
				if(_perso.getObjetByPos(Variable.ITEM_POS_ARME) == null)
					Packets.GAME_SEND_OT_PACKET(_out, -1);
				
				//Si objet de panoplie
				if(exObj.getTemplate().getPanopID() > 0)Packets.GAME_SEND_OS_PACKET(_perso,exObj.getTemplate().getPanopID());
			}//getNumbEquipedItemOfPanoplie(exObj.getTemplate().getPanopID()
			if(obj.getTemplate().getLevel() > _perso.get_lvl())
			{
				Packets.GAME_SEND_OAEL_PACKET(_out);
				return;
			}
			//On ne peut équiper 2 items de panoplies identiques, ou 2 Dofus identiques
			if(pos != Variable.ITEM_POS_NO_EQUIPED && (obj.getTemplate().getPanopID() != -1 || obj.getTemplate().getType() == Variable.ITEM_TYPE_DOFUS )&& _perso.hasEquiped(obj.getTemplate().getID()))
			return;
			...
			if(( obj2 = _perso.getSimilarItem(obj)) != null)
			{
				...
				Packets.GAME_SEND_REMOVE_ITEM_PACKET(_perso, obj.getGuid());
			}
			else
			{
				obj.setPosition(pos);
				Packets.GAME_SEND_OBJET_MOVE_PACKET(_perso,obj);
			...
			{
				Packets.GAME_SEND_PM_MOD_PACKET_TO_GROUP(_perso.getGroup(),_perso);
			}
			Packets.GAME_SEND_STATS_PACKET(_perso);
			if( pos == Variable.ITEM_POS_ARME 		||
				pos == Variable.ITEM_POS_COIFFE 	||
				pos == Variable.ITEM_POS_FAMILIER 	||
				pos == Variable.ITEM_POS_CAPE		||
				pos == Variable.ITEM_POS_BOUCLIER	||
				pos == Variable.ITEM_POS_NO_EQUIPED)
				Packets.GAME_SEND_ON_EQUIP_ITEM(_perso.get_curCarte(), _perso);
		
			//Si familier
			if(pos == Variable.ITEM_POS_FAMILIER && _perso.isOnMount())_perso.toogleOnMount();
			//Verif pour les outils de métier
			if(pos == Variable.ITEM_POS_NO_EQUIPED && _perso.getObjetByPos(Variable.ITEM_POS_ARME) == null)
				Packets.GAME_SEND_OT_PACKET(_out, -1);
			
			if(pos == Variable.ITEM_POS_ARME)
			{
				int ID = _perso.getObjetByPos(Variable.ITEM_POS_ARME).getTemplate().getID();
				for(Entry<Integer,StatsMetier> e : _perso.getMetiers().entrySet())
				{
					if(e.getValue().getTemplate().isValidTool(ID))
						Packets.GAME_SEND_OT_PACKET(_out,e.getValue().getTemplate().getId());
				}
			}
			//Si objet de panoplie
			if(obj.getTemplate().getPanopID() > 0)Packets.GAME_SEND_OS_PACKET(_perso,obj.getTemplate().getPanopID());
			
		}catch(Exception e)
		{
			e.printStackTrace();
			Packets.GAME_SEND_DELETE_OBJECT_FAILED_PACKET(_out);
		}
	}
	...
	private void Dialog_response(String packet)
	{
		...
		try
		{
			...
			{
				Packets.GAME_SEND_END_DIALOG_PACKET(_out);
	...
	private void Dialog_start(String packet)
	{
		try
		{
			...
			if( npc == null)return;
			Packets.GAME_SEND_DCK_PACKET(_out,npcID);
			...
			Packets.GAME_SEND_QUESTION_PACKET(_out,quest.parseToDQPacket(_perso));
	...
	private void Exchange_HDV(String packet)
	{
		...
		switch(packet.charAt(2))
		{
			case 'B': 	//Confirmation d'achat
				...
				ActionHouse curHdv = World.getHdv(Math.abs(_perso.get_isTradingWith()));
				...
				{
					Packets.GAME_SEND_EHm_PACKET(_perso,"-",ligneID+"");	//Enleve la ligne
					if(curHdv.getLigne(ligneID) != null && !curHdv.getLigne(ligneID).isEmpty())
						Packets.GAME_SEND_EHm_PACKET(_perso, "+", curHdv.getLigne(ligneID).parseToEHm()); //Réajoute la ligne si elle n'est pas vide
					...
					_perso.refreshStats();
					Packets.GAME_SEND_Ow_PACKET(_perso);
					Packets.GAME_SEND_Im_PACKET(_perso,"068");	//Envoie le message "Lot acheté"
				}
				else
				{
					Packets.GAME_SEND_Im_PACKET(_perso,"172");	//Envoie un message d'erreur d'achat
				}
			break;
			case 'l':	//Demande listage d'un template (les prix)
				...
				try
				{
					Packets.GAME_SEND_EHl(_perso,World.getHdv(Math.abs(_perso.get_isTradingWith())),templateID);
				}catch(NullPointerException e)	//Si erreur il y a, retire le template de la liste chez le client
				{
					Packets.GAME_SEND_EHM_PACKET(_perso,"-",templateID+"");
				}
				
			break;
			case 'P':	//Demande des prix moyen
				templateID = Integer.parseInt(packet.substring(3));
				Packets.GAME_SEND_EHP_PACKET(_perso,templateID);
			break;			
			case 'T':	//Demande des template de la catégorie
				...
				Packets.GAME_SEND_EHL_PACKET(_perso,categ,allTemplate);
			break;			
		}
	}
		...
		if(_perso.getInMountPark() != null)
		{
			...
			switch(c)
			{
				case 'C'://Parcho => Etable (Stocker)
					...
					if(DD == null)
					{
						int color = Variable.getMountColorByParchoTemplate(obj.getTemplate().getID());
						if(color <1)return;
						DD = new Mount(color);
					}
					...
					Packets.GAME_SEND_REMOVE_ITEM_PACKET(_perso,obj.getGuid());
					Packets.GAME_SEND_Ee_PACKET(_perso, '+', DD.parse());
				break;
				case 'c'://Etable => Parcho(Echanger)
					...
					//On créer le parcho
					ObjTemplate T = Variable.getParchoTemplateByMountColor(DD1.get_color());
					Item obj1 = T.createNewItem(1, false);
					//On efface les stats
					obj1.clearStats();
					...
					//Packets
					Packets.GAME_SEND_Ow_PACKET(_perso);
					Packets.GAME_SEND_Ee_PACKET(_perso,'-',DD1.get_id()+"");
				break;
				case 'g'://Equiper
					...
					//Packets
					Packets.GAME_SEND_Re_PACKET(_perso, "+", DD3);
					Packets.GAME_SEND_Ee_PACKET(_perso,'-',DD3.get_id()+"");
					Packets.GAME_SEND_Rx_PACKET(_perso);
				break;
				case 'p'://Equipé => Stocker
					...
					if(_perso.getMount()!=null?_perso.getMount().get_id() == guid:false)
					{
						...
						//Packets
						Packets.GAME_SEND_Ee_PACKET(_perso,'+',DD2.parse());
						Packets.GAME_SEND_Re_PACKET(_perso, "-", null);
						Packets.GAME_SEND_Rx_PACKET(_perso);
					}else//Sinon...
					{
			...
			switch(packet.charAt(2))
			{
				case 'G'://Kamas
					...
					if(kamas > 0)//Si On ajoute des kamas a la banque
					{
						...
						Packets.GAME_SEND_STATS_PACKET(_perso);
						Packets.GAME_SEND_EsK_PACKET(_perso,"G"+_perso.getBankKamas());
					}else
					{
		...
		if(_perso.get_isTradingWith() < 0)	//HDV
		{
			switch(packet.charAt(3))
			{
				case '-'://Retirer un objet de l'HDV
					...
					Packets.GAME_SEND_EXCHANGE_OTHER_MOVE_OK(_out,'-',"",cheapestID+"");
				break;
				case '+'://Mettre un objet en vente
					...
					{
						Packets.GAME_SEND_Im_PACKET(_perso, "176");
						return;
					}
					...
					Packets.GAME_SEND_STATS_PACKET(_perso);	//Met a jour les kamas du client
					...
					if(obj.getQuantity() > 1)	//Si c'est plusieurs objets ensemble enleve seulement la quantité de mise en vente
					{
						...
						Item newObj = obj.getTemplate().createNewItem(rAmount, false);
						newObj.clearStats();
					...
					else
					{
						...
						Packets.GAME_SEND_REMOVE_ITEM_PACKET(_perso,itmID);	//Envoie un packet au client pour retirer l'item de son inventaire
					}
					...
					Packets.GAME_SEND_EXCHANGE_OTHER_MOVE_OK(_out,'+',"",toAdd.parseToEmK());	//Envoie un packet pour ajouter l'item dans la fenetre de l'HDV du client
				break;
			}
	...
	{
		if(_perso.get_isTradingWith() == 0)return;
		Player target = World.getPersonnage(_perso.get_isTradingWith());
		if(target == null)return;
		Packets.GAME_SEND_EXCHANGE_CONFIRM_OK(_out,1);
		Packets.GAME_SEND_EXCHANGE_CONFIRM_OK(target.get_compte().getGameThread().get_out(),1);
	...
	private void Exchange_onSellItem(String packet)
	{
		try
		{
			...
			{
				Packets.GAME_SEND_SELL_ERROR_PACKET(_out);
				return;
			}
	...
	private void Exchange_onBuyItem(String packet)
	{
		...
		try
		{
			...
			if(template == null)//Si l'objet demandé n'existe pas(ne devrait pas arrivé)
			{
				...
				Packets.GAME_SEND_BUY_ERROR_PACKET(_out);
				return;
			}
			...
			Item newObj = template.createNewItem(qua,false);
			...
			Packets.GAME_SEND_BUY_OK_PACKET(_out);
			Packets.GAME_SEND_STATS_PACKET(_perso);
			Packets.GAME_SEND_Ow_PACKET(_perso);
		}catch(Exception e)
		{
	...
	private void Exchange_finish_buy(String packet)
	{
		...
		{
			Player p = World.getPersonnage(_perso.get_isTradingWith());
			if(p != null)
			{
				if(p.isOnline())
				{
					PrintWriter out = p.get_compte().getGameThread().get_out();
					Packets.GAME_SEND_EV_PACKET(out);
					p.set_isTradingWith(0);
				}
			}
		}
		
		Packets.GAME_SEND_EV_PACKET(_out);
		_perso.set_isTradingWith(0);
		_perso.set_away(false);
	...
	private void Exchange_start(String packet)
	{
		...
		{
			ActionHouse toOpen = World.getHdv(_perso.get_curCarte().get_id());
			...
			String info = "1,10,100;"+
						...
						";-1;"+
						toOpen.getSellTime();
			Packets.GAME_SEND_ECK_PACKET(_perso,11,info);
		...
		else if(packet.substring(2,4).equals("10"))	//Ouverture HDV vente
		{
			...
			String info = "1,10,100;"+
						...
						";-1;"+
						toOpen.getSellTime();
			Packets.GAME_SEND_ECK_PACKET(_perso,10,info);
			_perso.set_isTradingWith(0 - _perso.get_curCarte().get_id());	//Récupère l'ID de la map et rend cette valeur négative
			
			Packets.GAME_SEND_HDVITEM_SELLING(_perso);
		...
		switch(packet.charAt(2))
		{
			case '0'://Si NPC
				try
				{
					...
					NonPlayerCharacter.NPC npc = _perso.get_curCarte().getNPC(npcID);
					if(npc == null)return;
					Packets.GAME_SEND_ECK_PACKET(_out, 0, npcID+"");
					Packets.GAME_SEND_ITEM_VENDOR_LIST_PACKET(_out,npc);
			...
			case '1'://Si joueur
				try
				{
					...
					Player target = World.getPersonnage(guidTarget);
					if(target == null )
					{
						Packets.GAME_SEND_EXCHANGE_REQUEST_ERROR(_out,'E');
						return;
					}
					...
					{
						Packets.GAME_SEND_EXCHANGE_REQUEST_ERROR(_out,'O');
						return;
					}
					Packets.GAME_SEND_EXCHANGE_REQUEST_OK(_out, _perso.get_GUID(), guidTarget,1);
					Packets.GAME_SEND_EXCHANGE_REQUEST_OK(target.get_compte().getGameThread().get_out(),_perso.get_GUID(), guidTarget,1);
	...
	private void Environement_emote(String packet)
	{
		...
		Packets.GAME_SEND_eUK_PACKET_TO_MAP(_perso.get_curCarte(), _perso.get_GUID(), _perso.emoteActive());
	}
	...
	private void Environement_change_direction(String packet)
	{
		try
		{
			...
			Packets.GAME_SEND_eD_PACKET_TO_MAP(_perso.get_curCarte(),_perso.get_GUID(),dir);
		}catch(NumberFormatException e){return;};
	}
	...
	private void addToSpellBook(String packet)
	{
		try
		{
			...
			if(Spell != null)
			{
				_perso.set_SpellPlace(SpellID, Cipher.getHashedValueByInt(Position));
			}
	...
	private void boostSort(String packet)
	{
		try
		{
			...
			if(_perso.boostSpell(id))
			{
				...
				Packets.GAME_SEND_SPELL_UPGRADE_SUCCED(_out, id, _perso.getSortStatBySortIfHas(id).getLevel());
				Packets.GAME_SEND_STATS_PACKET(_perso);
			}else
			{
				GameServer.addToLog("Info: "+_perso.get_name()+": Echec BOOST sort id="+id);
				Packets.GAME_SEND_SPELL_UPGRADE_FAILED(_out);
				return;
			}
		}catch(NumberFormatException e){Packets.GAME_SEND_SPELL_UPGRADE_FAILED(_out);return;};
	}
		...
		try
		{
			switch(packet.charAt(1))
			{
				case 'D'://Détails d'un combat (liste des combats)
					...
					if(key == -1)return;
					Packets.GAME_SEND_FIGHT_DETAILS(_out,_perso.get_curCarte().get_fights().get(key));
				break;
				...
				case 'L'://Lister les combats
					Packets.GAME_SEND_FIGHT_LIST_PACKET(_out, _perso.get_curCarte());
				break;
	...
	public void Basic_state(String packet)
	{
		switch(packet.charAt(2))
		{
			case 'A': //Absent
				if(_perso.isAbsent)
				{
					Packets.GAME_SEND_Im_PACKET(_perso, "038");
					_perso.isAbsent = false;
				}
				else
				{
					Packets.GAME_SEND_Im_PACKET(_perso, "037");
			...
			case 'I': //Invisible
				...
				{
					Packets.GAME_SEND_Im_PACKET(_perso, "051");
					_perso.isInvisible = false;
				}
				else
				{
					Packets.GAME_SEND_Im_PACKET(_perso, "050");
	...
	private void Basic_chatMessage(String packet)
	{
		...
		if(_perso.isMuted())
		{
			Packets.GAME_SEND_MESSAGE(_perso,"Vous etes mute.", Main.CONFIG_MOTD_COLOR);
			return;
		}
		...
		switch(packet.charAt(2))
		{
			case '*'://Canal noir
				...
				if(_perso.get_fight() == null)
					Packets.GAME_SEND_cMK_PACKET_TO_MAP(_perso.get_curCarte(), "", _perso.get_GUID(), _perso.get_name(), msg);
				else
					Packets.GAME_SEND_cMK_PACKET_TO_FIGHT(_perso.get_fight(), 7, "", _perso.get_GUID(), _perso.get_name(), msg);
			...
			case '#'://Canal Equipe
				...
				if(_perso.get_fight() != null)
				{
					...
					if(team == -1)return;
					Packets.GAME_SEND_cMK_PACKET_TO_FIGHT(_perso.get_fight(), team, "#", _perso.get_GUID(), _perso.get_name(), msg);
			...
			case '$'://Canal groupe
				...
				msg = packet.split("\\|",2)[1];
				Packets.GAME_SEND_cMK_PACKET_TO_GROUP(_perso.getGroup(), "$", _perso.get_GUID(), _perso.get_name(), msg);
			break;
			...
			case ':'://Canal commerce
				...
				if((l = System.currentTimeMillis() - _timeLastTradeMsg) < Main.FLOOD_TIME)
				{
					...
					Packets.GAME_SEND_Im_PACKET(_perso, "0115;"+l);
					return;
				}
				_timeLastTradeMsg = System.currentTimeMillis();
				msg = packet.split("\\|",2)[1];
				Packets.GAME_SEND_cMK_PACKET_TO_ALL(":", _perso.get_GUID(), _perso.get_name(), msg);
			break;
			case '@'://Canal Admin
				if(_perso.get_compte().get_gmLvl() ==0)return;
				msg = packet.split("\\|",2)[1];
				Packets.GAME_SEND_cMK_PACKET_TO_ADMIN("@", _perso.get_GUID(), _perso.get_name(), msg);
			break;
			case '?'://Canal recrutement
				if(!_perso.get_canaux().contains(packet.charAt(2)+""))return;
				
				long L;
				if((L = System.currentTimeMillis() - _timeLastRecrutmentMsg) < Main.FLOOD_TIME)
				{
					L = (Main.FLOOD_TIME  - L)/1000;//On calcul la différence en secondes
			...
			case '%'://Canal guilde
				...
				if(_perso.get_guild() == null)return;
				msg = packet.split("\\|",2)[1];
				Packets.GAME_SEND_cMK_PACKET_TO_GUILD(_perso.get_guild(),"%", _perso.get_GUID(), _perso.get_name(), msg);
			break;
			case '!'://Canal Alignement
				if(!_perso.get_canaux().contains(packet.charAt(2)+""))return;
				if(_perso.get_align() == Variable.ALIGNEMENT_NEUTRE)return;
				
				msg = packet.split("\\|",2)[1];
				Packets.GAME_SEND_cMK_PACKET_TO_ALIGN(_perso.get_align(), _perso.get_GUID(), _perso.get_name(), msg);
			break;
			case 0xC2://Canal 
			break;
			default:
				...
				else
				{
					...
					{
						Packets.GAME_SEND_CHAT_ERROR_PACKET(_out, nom);
						return;
					}
					...
					{
						Packets.GAME_SEND_Im_PACKET(_perso, "114;"+target.get_name());
						return;
					}
					Packets.GAME_SEND_cMK_PACKET(target, "F", _perso.get_GUID(), _perso.get_name(), msg);
					Packets.GAME_SEND_cMK_PACKET(_perso, "T", target.get_GUID(), target.get_name(), msg);
				}
			break;
		}
	}
	...
	{
		Packets.GAME_SEND_SERVER_DATE(_out);
		Packets.GAME_SEND_SERVER_HOUR(_out);
	}
	...
	private void Game_on_showCase(String packet)
	{
		if(_perso == null)return;
		if(_perso.get_fight() == null)return;
		if(_perso.get_fight().get_state() != Variable.FIGHT_STATE_ACTIVE)return;
		int cellID = -1;
		try
		{
	...
	{
		if(_perso.get_fight() == null)return;
		if(_perso.get_fight().get_state() != Variable.FIGHT_STATE_PLACE)return;
		_perso.set_ready(packet.substring(2).equalsIgnoreCase("1"));
		_perso.get_fight().verifIfAllReady();
		Packets.GAME_SEND_FIGHT_PLAYER_READY_TO_FIGHT(_perso.get_fight(),3,_perso.get_GUID(),packet.substring(2).equalsIgnoreCase("1"));
			...
			case 1://Deplacement
				if(isOk)
				{
					...
					if(_perso.get_fight() == null)
					{
						...
						Case nextCell = _perso.get_curCarte().getCase(Cipher.cellCode_To_ID(path.substring(path.length()-2)));
						Case targetCell = _perso.get_curCarte().getCase(Cipher.cellCode_To_ID(GA._packet.substring(GA._packet.length()-2)));
						
						//On définie la case et on ajoute le personnage sur la case
						_perso.set_curCell(nextCell);
						_perso.set_orientation(Cipher.getIntByHashedValue(path.charAt(path.length()-3)));
						...
						if(targetCell.getObject() != null)
						{
							...
							{
								Variable.applyPlotIOAction(_perso,_perso.get_curCarte().get_id(),targetCell.getID());
							}
						}
	...
	{
		//Enclos
		Packets.GAME_SEND_Rp_PACKET(_perso, _perso.get_curCarte().getMountPark());
		//Objets sur la carte
		Packets.GAME_SEND_MAP_GMS_PACKETS(_perso.get_curCarte(), _perso);
		Packets.GAME_SEND_MAP_MOBS_GMS_PACKETS(_perso.get_compte().getGameThread().get_out(), _perso.get_curCarte());
		Packets.GAME_SEND_MAP_NPCS_GMS_PACKETS(_out,_perso.get_curCarte());
		Packets.GAME_SEND_MAP_OBJECTS_GDS_PACKETS(_out,_perso.get_curCarte());
		Packets.GAME_SEND_GDK_PACKET(_out);
		Packets.GAME_SEND_MAP_FIGHT_COUNT(_out, _perso.get_curCarte());
		//items au sol
	...
	private void game_aggro(String packet)
	{
		try
		{
			...
			Player target = World.getPersonnage(id);
			...
			_perso.toggleWings('+');
			Packets.GAME_SEND_GA_PACKET_TO_MAP(_perso.get_curCarte(),"", 906, _perso.get_GUID()+"", id+"");
			_perso.get_curCarte().newFight(_perso, target, Variable.FIGHT_TYPE_AGRESSION);
		}catch(Exception e){};
	}
	...
	private void game_join_fight(String packet)
	{
		...
		}else
		{
			try
			{
				...
				if(_perso.is_away()){Packets.GAME_SEND_GA903_ERROR_PACKET(_out,'o',guid);return;};
	...
	private void game_accept_duel(String packet)
	{
		...
		Packets.GAME_SEND_MAP_START_DUEL_TO_MAP(_perso.get_curCarte(),_perso.get_duelID(),_perso.get_GUID());
		Fight fight = _perso.get_curCarte().newFight(World.getPersonnage(_perso.get_duelID()),_perso,Variable.FIGHT_TYPE_CHALLENGE);
	...
	private void game_cancel_duel(String packet)
	{
		try
		{
			...
			Packets.GAME_SEND_CANCEL_DUEL_TO_MAP(_perso.get_curCarte(),_perso.get_duelID(),_perso.get_GUID());
	...
	private void game_ask_duel(String packet)
	{
		...
		{
			Packets.GAME_SEND_DUEL_Y_AWAY(_out, _perso.get_GUID());
			return;
		}
		try
		{
			int guid = Integer.parseInt(packet.substring(5));
			if(_perso.is_away() || _perso.get_fight() != null){Packets.GAME_SEND_DUEL_Y_AWAY(_out, _perso.get_GUID());return;}
			if(World.getPersonnage(guid) == null) return;
			if(World.getPersonnage(guid).is_away() || World.getPersonnage(guid).get_fight() != null || World.getPersonnage(guid).get_curCarte().get_id() != _perso.get_curCarte().get_id()){Packets.GAME_SEND_DUEL_E_AWAY(_out, _perso.get_GUID());return;}
			...
			Packets.GAME_SEND_MAP_NEW_DUEL_TO_MAP(_perso.get_curCarte(),_perso.get_GUID(),guid);
		}catch(NumberFormatException e){return;}
	}
	...
	private void game_parseDeplacementPacket(GameAction GA)
	{
		...
		if(_perso.get_fight() == null)
		{
			...
			if(result == 0)
			{
				Packets.GAME_SEND_GA_PACKET(_out,"", "0", "", "");
				removeAction(GA);
				return;
			}
			...
			if(result == -1000)
			{
				...
				path = Cipher.getHashedValueByInt(_perso.get_orientation())+Cipher.cellID_To_Code(_perso.get_curCell().getID());	
			}
			//On sauvegarde le path dans la variable
			GA._args = path;
			
			Packets.GAME_SEND_GA_PACKET_TO_MAP(_perso.get_curCarte(), ""+GA._id, 1, _perso.get_GUID()+"", "a"+Cipher.cellID_To_Code(_perso.get_curCell().getID())+path);
			addAction(GA);
	...
	private void parseAccountPacket(String packet)
	{
		switch(packet.charAt(1))
		{
			case 'A':
				String[] infos = packet.substring(2).split("\\|");
				if(Database.persoExist(infos[0]))
				{
					Packets.GAME_SEND_NAME_ALREADY_EXIST(_out);
					return;
				}
				...
				{
					Packets.GAME_SEND_CREATE_PERSO_FULL(_out);
					return;
				}
				...
				{
					Packets.GAME_SEND_CREATE_OK(_out);
					Packets.GAME_SEND_PERSO_LIST(_out, _compte.get_persos());
				}else
				{
					Packets.GAME_SEND_CREATE_FAILED(_out);
				}
			...
			case 'D':
				...
				reponse = Utils.parseReponse(reponse);
				...
				if(_compte.get_persos().containsKey(GUID))
				{
					if(_compte.get_persos().get(GUID).get_lvl() <20 ||(_compte.get_persos().get(GUID).get_lvl() >=20 && reponse.equals(_compte.get_reponse())))
					{
						...
						Packets.GAME_SEND_DELETE_PERSO_FAILED(_out);
				}else
					Packets.GAME_SEND_DELETE_PERSO_FAILED(_out);
			break;
			...
			case 'S':
				...
				Packets.GAME_SEND_PERSO_SELECTION_FAILED(_out);
			break;
			...
			case 'T':
				...
				if(_compte != null)
				{
					...
					Packets.GAME_SEND_ATTRIBUTE_SUCCESS(_out);
				}else
				{
					Packets.GAME_SEND_ATTRIBUTE_FAILED(_out);
				}
			break;
			
			case 'V':
				Packets.GAME_SEND_AV0(_out);
			break;
		}
	}
