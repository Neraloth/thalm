package org.thalm.game.character.item;
...
public class Item {
	...
	public static class ObjTemplate
	{
		...
		private ArrayList<WorldActions> onUseActions = new ArrayList<WorldActions>();
		private long sold;
		...
		public void addAction(WorldActions A)
		{
			onUseActions.add(A);
		}
		...
		{
			Item item = new Item(World.getNewItemGuid(), ID, qua, Variable.ITEM_POS_NO_EQUIPED, generateNewStatsFromTemplate(StrTemplate,useMax),getEffectTemplate(StrTemplate));
			return item;
		}
		...
		private Stats generateNewStatsFromTemplate(String statsTemplate,boolean useMax)
		{
			...
			{	
				...
				for(int a : Variable.ARMES_EFFECT_IDS)//Si c'est un Effet Actif
					if(a == statID)
						follow = false;
				...
				try
				{
					...
					if(useMax)
					{
						try
						{
							...
							value = min;
							if(max != 0)value = max;
						}catch(Exception e){value = Utils.getRandomJet(jet);};			
					}
				}catch(Exception e){};
		...
		private ArrayList<SpellEffect> getEffectTemplate(String statsTemplate)
		{
			...
			{	
				String[] stats = s.split("#");
				int statID = Integer.parseInt(stats[0],16);
				for(int a : Variable.ARMES_EFFECT_IDS)
				{
					if(a == statID)
					{
						int id = statID;
		...
		public void applyAction(Player perso,int objID)
		{
			for(WorldActions a : onUseActions)a.apply(perso,objID);
		}
	...
	protected int position = Variable.ITEM_POS_NO_EQUIPED;
	protected int guid;
	...
	public Item (int Guid, int template,int qua, int pos, String strStats)
	{
		this.guid = Guid;
	...
	public Item(int Guid, int template, int qua, int pos,	Stats stats,ArrayList<SpellEffect> effects)
	{
		this.guid = Guid;
		this.template = World.getObjTemplate(template);
		this.quantity = qua;
	...
	{	
		String posi = position==Variable.ITEM_POS_NO_EQUIPED?"":Integer.toHexString(position);
	...
	public String parseStatsString()
	{
		...
		{
			if(!isFirst)stats+=",";
			if(entry.getKey() == Variable.CAPTURE_MONSTRE)
	...
	public ArrayList<SpellEffect> getCritEffects()
	{
		...
		for(SpellEffect SE : Effects)
		{
			...
			{
				boolean boost = true;
				for(int i : Variable.NO_BOOST_CC_IDS)if(i == SE.getEffectID())boost = false;
				String[] infos = SE.getArgs().split(";");
				if(!boost)
				{
					effets.add(SE);
	...
	{
		Item ob = new Item(World.getNewItemGuid(), obj.getTemplate().getID(), qua,Variable.ITEM_POS_NO_EQUIPED, obj.getStats(), obj.getEffects());
		return ob;
	}
