...
public class ActionHouse {
	...
	public static class HdvEntry implements Comparable<HdvEntry>
	{
		...
		public HdvEntry(int price, byte amount, int owner, Item obj)
		{
			this._price = price;
			this._amount = amount;
			this._obj = obj;
	...
	public ActionHouse(int hdvID, float taxe, short sellTime, short maxItemCompte,short lvlMax, String categories)
	{
		this._hdvID = hdvID;
		this._taxe = taxe;
	...
	public synchronized boolean buyItem(int ligneID,byte amount, int price, Player newOwner)
	{
		...
		try
		{
			...
			Packets.GAME_SEND_STATS_PACKET(newOwner);	//Met a jour les kamas de l'acheteur
			...
			if(World.getCompte(toBuy.getOwner()) != null && World.getCompte(toBuy.getOwner()).get_curPerso() != null)
				Packets.GAME_SEND_Im_PACKET(World.getCompte(toBuy.getOwner()).get_curPerso(),
						"065;"+price+"~"+
			...
			if(toBuy.getOwner() == -1)
			{
				Database.SAVE_ITEM(toBuy.getObjet());
			}
			toBuy = null;
		}
		catch(NullPointerException e)
