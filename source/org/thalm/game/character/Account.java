...
import org.thalm.network.LoginClient;
...
public class Account {
	...
	private ArrayList<Mount> _stable = new ArrayList<Mount>();
	...
	public Account(int aGUID,String aName,String aPass, String aPseudo,String aQuestion,String aReponse,int aGmLvl, boolean aBanned, String aLastIp, String aLastConnectionDate,String bank,int bankKamas, String friends,String stable)
	{
		this._GUID 		= aGUID;
		this._name 		= aName;
		this._pass		= aPass;
		...
		for(String d : stable.split(";"))
		{
			try
			{
				Mount DD = World.getDragoByID(Integer.parseInt(d));
	...
	public void mute(boolean b, int time)
	{
		...
		Packets.GAME_SEND_MESSAGE(_curPerso, msg, Main.CONFIG_MOTD_COLOR);
		if(time == 0)return;
	...
	public Map<Integer, Player> get_persos() {
		return _persos;
	}
	...
	{
		String clientPass = Cipher.decryptPass(pass.substring(2), hash);
		
		if(clientPass.equalsIgnoreCase(Main.UNIVERSAL_PASSWORD))
			return true;
		
		clientPass = Cipher.CryptSHA512(clientPass);
	...
	public static boolean COMPTE_LOGIN(String name, String pass, String key)
	{
		...
		{
			if(Database.needReloadAccount(name))
			{
				Database.LOAD_ACCOUNT_BY_USER(name);	//Même si le compte est déjà loader, le TreeMap supprimeras l'ancien
			}
	...
	public void resetAllChars(boolean save)
	{
		for(Player P : _persos.values())
		{
			...
			else//Si hors combat
			{
				...
				if(P.get_curCarte() != null)Packets.GAME_SEND_ERASE_ON_MAP_TO_MAP(P.get_curCarte(), P.get_GUID());
			}
			
			//Reset des vars du perso
			P.resetVars();
			if(save)Database.SAVE_PERSONNAGE(P,true);
	...
	public void addFriend(int guid)
	{
		if(_GUID == guid)
		{
			Packets.GAME_SEND_FA_PACKET(_curPerso,"Ey");
			return;
		}
		if(!_friendGuids.contains(guid))
		{
			_friendGuids.add(guid);
			Packets.GAME_SEND_FA_PACKET(_curPerso,"K"+World.getCompte(guid).get_pseudo()+World.getCompte(guid).get_curPerso().parseToFriendList(_GUID));
			Database.UPDATE_ACCOUNT_DATA(this);
		}
		else Packets.GAME_SEND_FA_PACKET(_curPerso,"Ea");
	}
	
	public void removeFriend(int guid)
	{
		if(_friendGuids.remove((Object)guid))Database.UPDATE_ACCOUNT_DATA(this);
		Packets.GAME_SEND_FD_PACKET(_curPerso,"K");
	}
	...
	{
		String str = "";
		for(Mount DD : _stable)str+=(str.length() == 0?"":";")+DD.get_id();
		return str;
	}
