package org.thalm.game.character.job;
...
import org.thalm.game.map.Maps.InteractiveObject;
...
public class Job {
	...
	public static class StatsMetier
	{
		...
		public StatsMetier(int id,Job tp,int lvl,long xp)
		{
			...
			_lvl = lvl;
			_xp = xp;
			_posActions = Variable.getPosActionsToJob(tp.getId(),lvl);
		}
		...
		{
			_lvl++;
			_posActions = Variable.getPosActionsToJob(_template.getId(),_lvl);
			...
			if(send)
			{
				...
				list.add(this);
				Packets.GAME_SEND_JS_PACKET(P, list);
				Packets.GAME_SEND_STATS_PACKET(P);
				Packets.GAME_SEND_Ow_PACKET(P);
				Packets.GAME_SEND_JN_PACKET(P,_template.getId(),_lvl);
				Packets.GAME_SEND_JO_PACKET(P, list);
			}
		}
		...
		public void endAction(int id,Player P,InteractiveObject IO,GameAction GA,Case cell)
		{
			...
			addXp(P,_curAction.getXpWin()*Main.XP_METIER);
			//Packet JX
			//on créer la listes des statsMetier a envoyer (Seulement celle ci)
			ArrayList<StatsMetier> list = new ArrayList<StatsMetier>();
			list.add(this);
			Packets.GAME_SEND_JX_PACKET(P, list);
		}
	...
	public static class JobAction
	{
		...
		public void endAction(Player P, InteractiveObject IO, GameAction GA,Case cell)
		{
			if(!_isCraft)
			{
				...
				IO.setState(Variable.IOBJECT_STATE_EMPTY);
				IO.startTimer();
				//Packet GDF (changement d'état de l'IO)
				Packets.GAME_SEND_GDF_PACKET_TO_MAP(P.get_curCarte(), cell);
				
				boolean special = Utils.getRandomValue(0, 99)==0;//Restriction de niveau ou pas ?
				
				//On ajoute X ressources
				int qua = (_max>_min?Utils.getRandomValue(_min, _max):_min);
				int tID = Variable.getObjectByJobSkill(_skID,special);
								
				ObjTemplate T = World.getObjTemplate(tID);
				if(T == null)return;
				Item O = T.createNewItem(qua, false);
				...
				Packets.GAME_SEND_IQ_PACKET(P,P.get_GUID(),qua);
				Packets.GAME_SEND_Ow_PACKET(P);
				
				int maxPercent = 20+(P.getMetierBySkill(_skID).get_lvl()-20);
				if(P.getMetierBySkill(_skID).get_lvl() >= 20 && Utils.getRandomValue(1, maxPercent) == maxPercent)
				{
                    int[][] protectors = JOB_PROTECTORS;
                    for(int i = 0; i < protectors.length; i++)
                    {
                      	if(tID == protectors[i][1])
                       	{
                      		int monsterId = protectors[i][0];
                         	int monsterLvl = getProtectorLvl(P.get_lvl());            
                          	P.get_curCarte().startFigthVersusMonstres(P, new Monster.MobGroup(P.get_curCarte().get_nextObjectID(), cell.getID(), monsterId+","+monsterLvl+","+monsterLvl));
                            break;
                     	}
                    }
				}
		...
		public void startAction(Player P, InteractiveObject IO, GameAction GA,Case cell)
		{
			...
			if(!_isCraft)
			{
				...
				IO.setState(Variable.IOBJECT_STATE_EMPTYING);
				Packets.GAME_SEND_GA_PACKET_TO_MAP(P.get_curCarte(),""+GA._id, 501, P.get_GUID()+"", cell.getID()+","+_time);
				Packets.GAME_SEND_GDF_PACKET_TO_MAP(P.get_curCarte(),cell);
				_startTime = System.currentTimeMillis()+_time;//pour eviter le cheat
			}else
			{
				P.set_away(true);
				IO.setState(Variable.IOBJECT_STATE_EMPTYING);//FIXME trouver la bonne valeur
				P.setCurJobAction(this);
				Packets.GAME_SEND_ECK_PACKET(P, 3, _min+";"+_skID);//_min => Nbr de Case de l'interface
				Packets.GAME_SEND_GDF_PACKET_TO_MAP(P.get_curCarte(), cell);
			}
		}
		...
		public void modifIngredient(Player P,int guid, int qua)
		{
			...
			if(q > 0)
			{
				...
				Packets.GAME_SEND_EXCHANGE_MOVE_OK(P,'O', "+", guid+"|"+q);
			}else Packets.GAME_SEND_EXCHANGE_MOVE_OK(P,'O', "-", guid+"");
		}
		...
		public void craft()
		{
			...
			for(Entry<Integer,Integer> e : _ingredients.entrySet())
			{
				...
				if(newQua == 0)
				{
					...
					Packets.GAME_SEND_REMOVE_ITEM_PACKET(_P, e.getKey());
				}else
				{
					obj.setQuantity(newQua);
					Packets.GAME_SEND_OBJECT_QUANTITY_PACKET(_P, obj);
				}
			...
			Packets.GAME_SEND_Ow_PACKET(_P);
			...
			{
				Packets.GAME_SEND_Ec_PACKET(_P,"EI");
				Packets.GAME_SEND_IO_PACKET_TO_MAP(_P.get_curCarte(),_P.get_GUID(),"-");
				_ingredients.clear();
				
				return;
			}
			
			int chan =  Variable.getChanceByNbrCaseByLvl(SM.get_lvl(),_ingredients.size());
			int jet = Utils.getRandomValue(1, 100);
			boolean success = chan >= jet;
			
			if(!success)//Si echec
			{
				Packets.GAME_SEND_Ec_PACKET(_P,"EF");
				Packets.GAME_SEND_IO_PACKET_TO_MAP(_P.get_curCarte(),_P.get_GUID(),"-"+tID);
				Packets.GAME_SEND_Im_PACKET(_P, "0118");
			}else
			{
				Item newObj = World.getObjTemplate(tID).createNewItem(1, false);
				//Si signé on ajoute la ligne de Stat "Fabriqué par:"
				if(signed)newObj.addTxtStat(988, _P.get_name());
				boolean add = true;
				int guid = newObj.getGuid();
				
				for(Entry<Integer,Item> entry : _P.getItems().entrySet())
				{
				...
				if(add)
				{
					...
					Packets.GAME_SEND_OAKO_PACKET(_P,newObj);
					World.addObjet(newObj, true);
				}
				
				//on envoie les Packets
				Packets.GAME_SEND_Ow_PACKET(_P);
				Packets.GAME_SEND_Em_PACKET(_P,"KO+"+guid+"|1|"+tID+"|"+newObj.parseStatsString().replace(";","#"));
				Packets.GAME_SEND_Ec_PACKET(_P,"K;"+tID);
				Packets.GAME_SEND_IO_PACKET_TO_MAP(_P.get_curCarte(),_P.get_GUID(),"+"+tID);
			}
			
			
			//On donne l'xp
			int winXP =  Variable.calculXpWinCraft(SM.get_lvl(),_ingredients.size()) * Main.XP_METIER;
			if(success)
			{
				SM.addXp(_P,winXP);
				ArrayList<StatsMetier> SMs = new ArrayList<StatsMetier>();
				SMs.add(SM);
				Packets.GAME_SEND_JX_PACKET(_P, SMs);
			}
		...
		private void doFmCraft()
		{
			...
			if(isElementChanging > 0)chan = Utils.calculElementChangeChance(SM.get_lvl(), obj.getTemplate().getLevel(), isElementChanging);
			//else TODO;
			...
			{
				//FIXME
				Packets.GAME_SEND_Em_PACKET(_P,"EO+"+_P.get_GUID()+"|1|"+tID+"|"+obj.parseStatsString().replace(";","#"));
				Packets.GAME_SEND_Ec_PACKET(_P,"EF");
				Packets.GAME_SEND_IO_PACKET_TO_MAP(_P.get_curCarte(),_P.get_GUID(),"-"+tID);
				Packets.GAME_SEND_Im_PACKET(_P, "0118");
			}else
			{
				...
				//On envoie les packets
				Packets.GAME_SEND_Ow_PACKET(_P);
				Packets.GAME_SEND_Em_PACKET(_P,"KO+"+_P.get_GUID()+"|1|"+tID+"|"+obj.parseStatsString().replace(";","#"));
				Packets.GAME_SEND_Ec_PACKET(_P,"K;"+tID);
				Packets.GAME_SEND_IO_PACKET_TO_MAP(_P.get_curCarte(),_P.get_GUID(),"+"+tID);
			}
			...
			if(sign != null)
			{
				...
				if(newQua <= 0)
				{
					...
					Packets.GAME_SEND_REMOVE_ITEM_PACKET(_P, sign.getGuid());
				}else
				{
					sign.setQuantity(newQua);
					Packets.GAME_SEND_OBJECT_QUANTITY_PACKET(_P, sign);
				}
			}
			//Objet modificateur
			if(mod != null)
			{
				...
				if(newQua <= 0)
				{
					...
					Packets.GAME_SEND_REMOVE_ITEM_PACKET(_P, mod.getGuid());
				}else
				{
					mod.setQuantity(newQua);
					Packets.GAME_SEND_OBJECT_QUANTITY_PACKET(_P, mod);
				}
			}
			//fin
		...
		public void repeat(int time,Player P)
		{
			...
			{
				Packets.GAME_SEND_EA_PACKET(P,a+"");
				_ingredients.clear();
				_ingredients.putAll(_lastCraft);
				craft();
			}
			Packets.GAME_SEND_Ea_PACKET(P, "1");
		}
		...
		public void putLastCraftIngredients()
		{
			...
			for(Entry<Integer,Integer> e : _ingredients.entrySet())
			{
				...
				Packets.GAME_SEND_EXCHANGE_MOVE_OK(_P,'O', "+", e.getKey()+"|"+e.getValue());
			}
		}
