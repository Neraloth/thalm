...
import org.thalm.common.Condition;
...
public class WorldActions {
	...
	public WorldActions(int id, String args, String cond)
	{
		this.ID = id;
		this.args = args;
		this.cond = cond;
	...
	public void apply(Player perso, int objetID)
	{
		if(perso == null)return;
		if(!cond.equalsIgnoreCase("") && !cond.equalsIgnoreCase("-1")&& !Condition.validConditions(perso,cond))
		{
			Packets.GAME_SEND_Im_PACKET(perso, "119");
			return;
		}
		...
		switch(ID)
		{
			case -2://créer guilde
				...
				{
					Packets.GAME_SEND_gC_PACKET(perso, "Ea");
					return;
				}
			...
			case -1://Ouvrir banque
				...
				if(cost > 0)
				{
					...
					Packets.GAME_SEND_Im_PACKET(perso, "020;"+cost);
				}
				Packets.GAME_SEND_ECK_PACKET(perso.get_compte().getGameThread().get_out(), 5, "");
				Packets.GAME_SEND_EL_BANK_PACKET(perso);
				perso.set_away(true);
			...
			case 1://Discours NPC
				...
				{
					Packets.GAME_SEND_END_DIALOG_PACKET(out);
				...
				}else
				{
					...
					Packets.GAME_SEND_QUESTION_PACKET(out, quest.parseToDQPacket(perso));
				}
			break;
			...
			case 5://objet
				try
				{
					...
					if(perso.isOnline())//on envoie le packet qui indique l'ajout//retrait d'un item
					{
						...
						{
							if(count >= 0){
								Packets.GAME_SEND_Im_PACKET(perso, "021;"+count+"~"+tID);
							}
							else if(count < 0){
								Packets.GAME_SEND_Im_PACKET(perso, "022;"+-count+"~"+tID);
							}
						}
					}
				}catch(Exception e){GameServer.addToLog(e.getMessage());};
			...
			case 8://Ajouter une Stat
				try
				{
					...
					switch(statID)
					{
						case Variable.STATS_ADD_INTE: messID = 14;break;
					}
					if(messID>0)
						Packets.GAME_SEND_Im_PACKET(perso, "0"+messID+";"+number);
				}catch(Exception e ){return;};
			break;
			...
			case 10://Pain/potion/viande/poisson
				try
				{
					...
					if(max == 0) max = min;
					int val = Utils.getRandomValue(min, max);
			...
			case 11://Definir l'alignement
				try
				{
					...
					if(perso.get_align() != Variable.ALIGNEMENT_NEUTRE && !replace)return;
			...
			case 12://Spawn Groupe args : boolean delObj?,boolean enArène?
				try
				{
					...
					SoulStone pierrePleine = (SoulStone)World.getObjet(objetID);
