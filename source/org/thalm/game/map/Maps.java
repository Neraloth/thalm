...
import org.thalm.game.character.guild.Guild.Percepteur;
...
public class Maps {
	...
	private Map<Integer,ArrayList<WorldActions>> _endFightAction = new TreeMap<Integer,ArrayList<WorldActions>>();
	...
	public static class MountPark
	{
		...
		public MountPark(int owner, Maps map, int size,String data, int guild,int price)
		{
			_owner = owner;
			_door = map.getMountParkDoor();
			_size = size;
			_guild = World.getGuild(guild);//TODO:World.getGuild(guild);
			_map = map;
			_price = price;
	...
	public static class InteractiveObject
	{
		...
		public InteractiveObject(Maps a_map,Case a_cell,int a_id)
		{
			...
			_map = a_map;
			_cell = a_cell;
			_state = Variable.IOBJECT_STATE_FULL;
			...
			_respawnTimer = new Timer(respawnTime,
					new ActionListener()
					{
						...
						{
							_respawnTimer.stop();
							_state = Variable.IOBJECT_STATE_FULLING;
							_interactive = true;
							Packets.GAME_SEND_GDF_PACKET_TO_MAP(_map, _cell);
							_state = Variable.IOBJECT_STATE_FULL;
						}
					}
			);
		...
		public void startTimer()
		{
			...
			_state = Variable.IOBJECT_STATE_EMPTY2;
		...
		public boolean isWalkable()
		{
			...
			return _template.isWalkable() && _state == Variable.IOBJECT_STATE_FULL;
		}
	}
	
	public Map<Integer, Case> getCell() {
		return _cases;
	}
	...
	public static class Case
	{
		...
		private ArrayList<WorldActions> _onCellStop;// = new ArrayList<Action>();
		...
		public boolean canDoAction(int id)
		{
			switch(id)
			{
				...
				case 45:
					...
					{
						case 7511://Blé
							return _object.getState() == Variable.IOBJECT_STATE_FULL;
					}
				return false;
				//Faucher Orge
				case 53:
		...
		{
			if(_onCellStop == null)
				_onCellStop = new ArrayList<WorldActions>();
			
			_onCellStop.add(new WorldActions(id,args,cond));
		}
		...
		public void applyOnCellStopActions(Player perso)
		{
			...
			for(WorldActions act : _onCellStop)
			{
				act.apply(perso,-1);
			}
		}
		...
		public void startAction(Player perso, GameAction GA)
		{
			...
			if(Variable.isJobAction(actionID))
			{
			...
			switch(actionID)
			{
				case 44://Sauvegarder pos
					...
					Packets.GAME_SEND_Im_PACKET(perso, "06");
				break;
			
				case 102://Puiser
					if(!_object.isInteractive())return;//Si l'objet est utilisé
					if(_object.getState() != Variable.IOBJECT_STATE_FULL)return;//Si le puits est vide
					_object.setState(Variable.IOBJECT_STATE_EMPTYING);
					_object.setInteractive(false);
					Packets.GAME_SEND_GA_PACKET_TO_MAP(perso.get_curCarte(),""+GA._id, 501, perso.get_GUID()+"", _id+","+_object.getUseDuration()+","+_object.getUnknowValue());
					Packets.GAME_SEND_GDF_PACKET_TO_MAP(perso.get_curCarte(),this);
				break;
				...
				case 175://Acceder a un enclos
					if(_object.getState() != Variable.IOBJECT_STATE_EMPTY);
				...
				case 183://Retourner sur Incarnam
					if(perso.get_lvl()>15)
					{
						Packets.GAME_SEND_Im_PACKET(perso, "1127");
						perso.get_compte().getGameThread().removeAction(GA);
						return;
					}
					int mapID  = Variable.getStartMap(perso.get_classe());
					int cellID = Variable.getStartCell(perso.get_classe());
		...
		public void finishAction(Player perso, GameAction GA)
		{
			...
			switch(actionID)
			{
				...
				case 102://Puiser
					_object.setState(Variable.IOBJECT_STATE_EMPTY);
					_object.setInteractive(false);
					_object.startTimer();
					Packets.GAME_SEND_GDF_PACKET_TO_MAP(perso.get_curCarte(),this);
					int qua = Utils.getRandomValue(1, 10);//On a entre 1 et 10 eaux
					Item obj = World.getObjTemplate(311).createNewItem(qua, false);
					if(perso.addObjet(obj, true))
						World.addObjet(obj,true);
					Packets.GAME_SEND_IQ_PACKET(perso,perso.get_GUID(),qua);
				break;
	...
	public Maps(int _id, String _date, int _w, int _h, String _key, String places,String dData,String monsters,String mapPos,int maxGroup,int maxSize)	//MARTHIEUBEAN
	{
		this._id = _id;
		this._date = _date;
		this._w = _w;
		...
		if(Main.CONFIG_COMPILED_MAP)
		{
			_cases = Cipher.DecompileMapData(this,dData);
		}
		else
		{
		...
		for(int id : Variable.NO_MOBS_MAP)
		{
			if(id == _id)return;
		}
		...
		{
			spawnGroup(Variable.ALIGNEMENT_NEUTRE,_maxGroup,false,-1);//Spawn des groupes d'alignement neutre 
			spawnGroup(Variable.ALIGNEMENT_BONTARIEN,1,false,-1);//Spawn du groupe de gardes bontarien s'il y a
			spawnGroup(Variable.ALIGNEMENT_BRAKMARIEN,1,false,-1);//Spawn du groupe de gardes brakmarien s'il y a
		}
	}
	...
	public void applyEndFightAction(int type,Player perso)
	{
		...
		for(WorldActions A : _endFightAction.get(type))A.apply(perso,-1);
	}
	public void addEndFightAction(int type,WorldActions A)
	{
		if(_endFightAction.get(type) == null)_endFightAction.put(type, new ArrayList<WorldActions>());
	...
	{
		if(_endFightAction.get(type) == null)return;
		ArrayList<WorldActions> copy = new ArrayList<WorldActions>();
		copy.addAll(_endFightAction.get(type));
		for(WorldActions A : copy)if(A.getID() == aType)_endFightAction.get(type).remove(A);
	}
	...
	public Maps(int id, String date, int w, int h, String key, String places)
	{
		_id = id;
		_date = date;
		_w = w;
	...
	{
		NonPlayerCharacter temp = World.getNPCTemplate(npcID);
		if(temp == null)return null;
		if(getCase(cellID) == null)return null;
		NPC npc = new NPC(temp,get_nextObjectID(),cellID,dir);
	...
	public void spawnGroup(int align, int nbr,boolean log,int cellID)
	{
		...
		for(int a = 1; a<=nbr;a++)
		{
			MobGroup group  = new MobGroup(get_nextObjectID(),align,_mobPossibles,this,cellID,this._maxSize);
			if(group.getMobs().size() == 0)continue;
			_mobGroups.put(get_nextObjectID(), group);
			if(log)
			{
				GameServer.addToLog("Groupe de monstres ajoutés sur la map: "+_id+" alignement: "+align+" ID: "+get_nextObjectID());
				Packets.GAME_SEND_MAP_MOBS_GM_PACKET(this, group);
			}
			set_nextObjectID(get_nextObjectID() - 1);
		}
	}
	public void spawnGroup(boolean timer,boolean log,int cellID,String groupData,String condition)
	{
		...
		if(log)
		{
			GameServer.addToLog("Groupe de monstres ajoutés sur la map: "+_id+" ID: "+get_nextObjectID());
		}
		Packets.GAME_SEND_MAP_MOBS_GM_PACKET(this, group);
		set_nextObjectID(get_nextObjectID() - 1);
	...
	{
		Packets.GAME_SEND_ADD_PLAYER_TO_MAP(this,perso);
	...
	public String getGMsPackets()
	{
		...
		for(Case cell : _cases.values())for(Player perso : cell.getPersos().values())packets += "GM|+"+perso.parseToGM()+'\u0000';
		return packets;
	}
	...
	public Fight newFight(Player init1,Player init2,int type)
	{
		...
		_fights.put(id,f);
		Packets.GAME_SEND_MAP_FIGHT_COUNT_TO_MAP(this);
		return f;
	}
	...
	public int getRandomFreeCellID()
	{
		...
		int rand = Utils.getRandomValue(0, freecell.size()-1);
	...
	public void refreshSpawns()
	{
		...
		{
			Packets.GAME_SEND_ERASE_ON_MAP_TO_MAP(this, id);
		}
		_mobGroups.clear();
		_mobGroups.putAll(_fixMobGroups);
		for(MobGroup mg : _fixMobGroups.values())Packets.GAME_SEND_MAP_MOBS_GM_PACKET(this, mg);

		spawnGroup(Variable.ALIGNEMENT_NEUTRE,_maxGroup,true,-1);//Spawn des groupes d'alignement neutre 
		spawnGroup(Variable.ALIGNEMENT_BONTARIEN,1,true,-1);//Spawn du groupe de gardes bontarien s'il y a
		spawnGroup(Variable.ALIGNEMENT_BRAKMARIEN,1,true,-1);//Spawn du groupe de gardes brakmarien s'il y a
	}
	...
	public void onPlayerArriveOnCell(Player perso,int caseID)
	{
		...
		if(obj != null)
		{
			...
			Packets.GAME_SEND_GDO_PACKET_TO_MAP(this,'-',caseID,0,0);
			Packets.GAME_SEND_Ow_PACKET(perso);
		...
		for(MobGroup group : _mobGroups.values())
		{
			if(Pathfinding.getDistanceBetween(this,caseID,group.getCellID()) <= group.getAggroDistance())//S'il y aggro
			{
				if((group.getAlignement() == -1 
						...
						&& Condition.validConditions(perso, group.getCondition()))
				{
	...
	public void startFigthVersusMonstres(Player perso, MobGroup group)
	{
		...
		else Packets.GAME_SEND_MAP_MOBS_GMS_PACKETS_TO_MAP(this);
	...
	public void sendFloorItems(Player perso)
	{
		for(Case c : _cases.values())
		{
			...
			Packets.GAME_SEND_GDO_PACKET(perso,'+',c.getID(),c.getDroppedItem().getTemplate().getID(),0);
		}
	}
