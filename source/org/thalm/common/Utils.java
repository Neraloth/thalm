...
import org.thalm.game.World.Couple;
...
public class Utils {
	...
	{
		return (Utils.getRandomValue(0, 3)*2)+1;
	}
	/*FIN*/
	...
	{
		int agiTR = tacleur.getTotalStats().getEffect(Variable.STATS_ADD_AGIL);
		int agiT = tacle.getTotalStats().getEffect(Variable.STATS_ADD_AGIL);
		int a = agiTR+25;
		int b = agiTR+agiT+50;
	...
	public static long getXpWinPvm3(Fighter perso, ArrayList<Fighter> winners,ArrayList<Fighter> loosers,long groupXP)
	{
		...
		{
			float sag = perso.getTotalStats().getEffect(Variable.STATS_ADD_SAGE);
			float coef = (sag + 100)/100;
			int taux = Main.XP_PVM;
			long xpWin = 0;
			int lvlmax = 0;
	...
	{
		int statC = caster.getTotalStats().getEffect(Variable.STATS_ADD_INTE);
		int soins = caster.getTotalStats().getEffect(Variable.STATS_ADD_SOIN);
		if(statC<0)statC=0;
	...
	public static int calculFinalDommage(Fight fight,Fighter caster,Fighter target,int statID,int jet,boolean isHeal)
	{
		...
		domC = caster.getTotalStats().getEffect(Variable.STATS_ADD_DOMA);
		perdomC = caster.getTotalStats().getEffect(Variable.STATS_ADD_PERDOM);
		int multiplier = caster.getTotalStats().getEffect(Variable.STATS_MULTIPLY_DOMMAGE);
		switch(statID)
		{
			case Variable.ELEMENT_NULL://Fixe
				statC = 0;
				resfT = 0;
				respT = 0;
				respT = 0;
			break;
			case Variable.ELEMENT_NEUTRE://neutre
				statC = caster.getTotalStats().getEffect(Variable.STATS_ADD_FORC);
				resfT = target.getTotalStats().getEffect(Variable.STATS_ADD_R_NEU);
				respT = target.getTotalStats().getEffect(Variable.STATS_ADD_RP_NEU);
				if(caster.getPersonnage() != null)//Si c'est un joueur
				{
					respT += target.getTotalStats().getEffect(Variable.STATS_ADD_RP_PVP_NEU);
					resfT += target.getTotalStats().getEffect(Variable.STATS_ADD_R_PVP_NEU);
				}
			...
			break;
			case Variable.ELEMENT_TERRE://force
				statC = caster.getTotalStats().getEffect(Variable.STATS_ADD_FORC);
				resfT = target.getTotalStats().getEffect(Variable.STATS_ADD_R_TER);
				respT = target.getTotalStats().getEffect(Variable.STATS_ADD_RP_TER);
				if(caster.getPersonnage() != null)//Si c'est un joueur
				{
					respT += target.getTotalStats().getEffect(Variable.STATS_ADD_RP_PVP_TER);
					resfT += target.getTotalStats().getEffect(Variable.STATS_ADD_R_PVP_TER);
				}
				...
				resfT = target.getTotalStats().getEffect(184);
			break;
			case Variable.ELEMENT_EAU://chance
				statC = caster.getTotalStats().getEffect(Variable.STATS_ADD_CHAN);
				resfT = target.getTotalStats().getEffect(Variable.STATS_ADD_R_EAU);
				respT = target.getTotalStats().getEffect(Variable.STATS_ADD_RP_EAU);
				if(caster.getPersonnage() != null)//Si c'est un joueur
				{
					respT += target.getTotalStats().getEffect(Variable.STATS_ADD_RP_PVP_EAU);
					resfT += target.getTotalStats().getEffect(Variable.STATS_ADD_R_PVP_EAU);
				}
				//Ajout de la resist Magique
				resfT = target.getTotalStats().getEffect(183);
			break;
			case Variable.ELEMENT_FEU://intell
				statC = caster.getTotalStats().getEffect(Variable.STATS_ADD_INTE);
				resfT = target.getTotalStats().getEffect(Variable.STATS_ADD_R_FEU);
				respT = target.getTotalStats().getEffect(Variable.STATS_ADD_RP_FEU);
				if(caster.getPersonnage() != null)//Si c'est un joueur
				{
					respT += target.getTotalStats().getEffect(Variable.STATS_ADD_RP_PVP_FEU);
					resfT += target.getTotalStats().getEffect(Variable.STATS_ADD_R_PVP_FEU);
				}
				//Ajout de la resist Magique
				resfT = target.getTotalStats().getEffect(183);
			break;
			case Variable.ELEMENT_AIR://agilité
				statC = caster.getTotalStats().getEffect(Variable.STATS_ADD_AGIL);
				resfT = target.getTotalStats().getEffect(Variable.STATS_ADD_R_AIR);
				respT = target.getTotalStats().getEffect(Variable.STATS_ADD_RP_AIR);
				if(caster.getPersonnage() != null)//Si c'est un joueur
				{
					respT += target.getTotalStats().getEffect(Variable.STATS_ADD_RP_PVP_AIR);
					resfT += target.getTotalStats().getEffect(Variable.STATS_ADD_R_PVP_AIR);
				}
				//Ajout de la resist Magique
				resfT = target.getTotalStats().getEffect(183);
			break;
		...
		//Renvoie
		int renvoie = target.getTotalStatsLessBuff().getEffect(Variable.STATS_RETDOM);
		if(renvoie >0 && !isHeal)
		{
		...
		if(!isHeal)num -= armor;
		if(!isHeal)if(armor > 0)Packets.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 105, caster.getGUID()+"", target.getGUID()+","+armor);
	...
	private static int getArmorResist(Fighter target, int statID)
	{
		...
		for(SpellEffect SE : target.getBuffsByEffectID(265))
		{
			...
			switch(SE.getSpell())
			{
				case 1://Armure incandescente
					...
					if(statID != Variable.ELEMENT_FEU)continue;
				...
				case 6://Armure Terrestre
					...
					if(statID != Variable.ELEMENT_TERRE && statID != Variable.ELEMENT_NEUTRE)continue;
					//Les stats du féca sont prises en compte
					fighter = SE.getCaster();
				break;
				case 14://Armure Venteuse
					//Si pas element air, on ignore l'armure
					if(statID != Variable.ELEMENT_AIR)continue;
					//Les stats du féca sont prises en compte
					fighter = SE.getCaster();
				break;
				case 18://Armure aqueuse
					//Si pas element eau, on ignore l'armure
					if(statID != Variable.ELEMENT_EAU)continue;
					//Les stats du féca sont prises en compte
					fighter = SE.getCaster();
				break;
			...
			int intell = fighter.getTotalStats().getEffect(Variable.STATS_ADD_INTE);
			int carac = 0;
			switch(statID)
			{
				case Variable.ELEMENT_AIR:
					carac = fighter.getTotalStats().getEffect(Variable.STATS_ADD_AGIL);
				break;
				case Variable.ELEMENT_FEU:
					carac = fighter.getTotalStats().getEffect(Variable.STATS_ADD_INTE);
				break;
				case Variable.ELEMENT_EAU:
					carac = fighter.getTotalStats().getEffect(Variable.STATS_ADD_CHAN);
				break;
				case Variable.ELEMENT_NEUTRE:
				case Variable.ELEMENT_TERRE:
					carac = fighter.getTotalStats().getEffect(Variable.STATS_ADD_FORC);
				break;
			}
		...
		{
			int intell = target.getTotalStats().getEffect(Variable.STATS_ADD_INTE);
			int carac = 0;
			switch(statID)
			{
				case Variable.ELEMENT_AIR:
					carac = target.getTotalStats().getEffect(Variable.STATS_ADD_AGIL);
				break;
				case Variable.ELEMENT_FEU:
					carac = target.getTotalStats().getEffect(Variable.STATS_ADD_INTE);
				break;
				case Variable.ELEMENT_EAU:
					carac = target.getTotalStats().getEffect(Variable.STATS_ADD_CHAN);
				break;
				case Variable.ELEMENT_NEUTRE:
				case Variable.ELEMENT_TERRE:
					carac = target.getTotalStats().getEffect(Variable.STATS_ADD_FORC);
				break;
			}
			int value = SE.getValue();
			int a = value * (100 + (int)(intell/2) + (int)(carac/2))/100;
	...
	{
		int esquiveC = c=='a'?caster.getTotalStats().getEffect(Variable.STATS_ADD_AFLEE):caster.getTotalStats().getEffect(Variable.STATS_ADD_MFLEE);
		int esquiveT = c=='a'?target.getTotalStats().getEffect(Variable.STATS_ADD_AFLEE):target.getTotalStats().getEffect(Variable.STATS_ADD_MFLEE);
		int ptsMax = c=='a'?caster.getTotalStatsLessBuff().getEffect(Variable.STATS_ADD_PA):caster.getTotalStatsLessBuff().getEffect(Variable.STATS_ADD_PM);
		int retrait = 0;
		
		for(int i = 0; i < value;i++)
		{
			int ptsAct = c=='a'?target.getTotalStats().getEffect(Variable.STATS_ADD_PA):target.getTotalStats().getEffect(Variable.STATS_ADD_PM);
	...
	public static long getXpWinPvm(Fighter perso, ArrayList<Fighter> team,ArrayList<Fighter> loose, long groupXP)
	{
		...
		float bonusSage = (perso.getTotalStats().getEffect(Variable.STATS_ADD_SAGE)+100)/100;
		/* Formule 1
	...
	public static int getKamasWin(Fighter i, ArrayList<Fighter> winners, int maxk, int mink)
	{
		...
		return rkamas*Main.KAMAS;
	}
	
	public static int getZaapCost(Maps map, Maps map2)
	{
		int cost = 0;
	...
	public static int calculDeshonorWin(ArrayList<Fighter> winners,ArrayList<Fighter> loosers,Fighter F)
	{
		...
		if(F.getPersonnage().get_align() == Variable.ALIGNEMENT_NEUTRE || F.getPersonnage().get_align() == Variable.ALIGNEMENT_MERCENAIRE)return 0;
		
		for(Fighter f : ennemy)
		{
			if(f.getPersonnage() == null)continue;
			if(f.getPersonnage().get_align() == Variable.ALIGNEMENT_NEUTRE)return 1;
		}
		return 0;
	}
	...
	public static int getCellIdNotUsed(Player player) {
		Maps map = player.get_curCarte();
		int weight = player.get_curCarte().get_w();
		int cell = player.get_curCell().getID();
		
		int front = (cell + weight);
		int back = (cell - weight);
		int right = (cell + (weight - 1));
		int left = (cell - (weight - 1));
		
		if (map.getCase(front).getDroppedItem() == null && map.getCell().get(front).getPersos().isEmpty()
		&& map.getCell().get(front).isWalkable(false)) {
			return front;
			
		} else if (map.getCase(back).getDroppedItem() == null && map.getCell().get(back).getPersos().isEmpty()
		&& map.getCell().get(back).isWalkable(false)) {
			return back;
			
		} else if (map.getCase(right).getDroppedItem() == null && map.getCell().get(right).getPersos().isEmpty()
		&& map.getCell().get(right).isWalkable(false)) {
			return right;
			
		} else if (map.getCase(left).getDroppedItem() == null && map.getCell().get(left).getPersos().isEmpty()
		&& map.getCell().get(left).isWalkable(false)) {
			return left;
		}
		return 0;
	}
