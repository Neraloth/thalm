...
public class Packets {
	...
	public static String REALM_SEND_HC_PACKET(PrintWriter out)
	{
		...
		if(Main.CONFIG_DEBUG)
			LoginServer.addToSockLog("Realm: Send>>"+packet);
		return hashkey;
	}
	
	public static void REALM_SEND_REQUIRED_VERSION(PrintWriter out)
	{
		String packet = "AlEv" +Variable.CLIENT_VERSION;
		send(out,packet);
		if(Main.CONFIG_DEBUG)
			LoginServer.addToSockLog("Conn: Send>>"+packet);
	}
	...
	public static void send(Player p, String packet)
	{
		...
		{
			packet = Cipher.toUtf(packet);	//MARTHIEUBEAN
			...
			int queueID)
	{
		...
		send(out,packet);
		if(Main.CONFIG_DEBUG)
			LoginServer.addToSockLog("Serv: Send>>"+packet);
	}
	...
	public static void REALM_SEND_GAME_SERVER_IP(PrintWriter out,int guid,boolean isHost)
	{
		...
		{
			String ip = Main.CONFIG_IP_LOOPBACK && isHost?Cipher.CryptIP("127.0.0.1")+Cipher.CryptPort(Main.CONFIG_GAME_PORT):Main.GAMESERVER_IP;
			packet += "XK"+ip+guid;
		}else
		{
			String ip = Main.CONFIG_IP_LOOPBACK && isHost?"127.0.0.1":Main.IP;
			packet += "YK"+ip+":"+Main.CONFIG_GAME_PORT+";"+guid;
		}
		send(out,packet);
		if(Main.CONFIG_DEBUG)
			LoginServer.addToSockLog("Conn: Send>>"+packet);
	...
	public static void GAME_SEND_ASK(PrintWriter out,Player perso)
	{
		String packet = "ASK|"+
		perso.get_GUID()+"|"+
		perso.get_name()+"|"+
	...
	public static void GAME_SEND_MAP_MOBS_GMS_PACKETS(PrintWriter out, Maps carte)
	{
	...
	public static void GAME_SEND_MAP_OBJECTS_GDS_PACKETS(PrintWriter out, Maps carte)
	{
	...
	public static void GAME_SEND_MAP_NPCS_GMS_PACKETS(PrintWriter out, Maps carte)
	{
	...
	public static void GAME_SEND_MAP_GMS_PACKETS(PrintWriter out, Maps carte)
	{
	...
	public static void GAME_SEND_ADD_PLAYER_TO_MAP(Maps map, Player perso)
	{
		...
		for(Player P : map.getPersos())send(P,packet);	
		if(Main.CONFIG_DEBUG)
			GameServer.addToSockLog("Game: Map "+map.get_id()+": Send>>"+packet);
	}
	...
	public static void GAME_SEND_MAP_FIGHT_COUNT(PrintWriter out,Maps map)
	{
	...
	{
		String packet = "cs<font color='#"+color+"'>"+mess+"</font>";
		for(Player perso : map.getPersos())
		{
			send(perso,packet);
		}
		if(Main.CONFIG_DEBUG)
	...
	public static void GAME_SEND_FIGHT_LIST_PACKET(PrintWriter out,Maps map)
	{
		String packet = "fL";
	...
	{
		String packet = "cMK"+suffix+"|"+guid+"|"+name+"|"+msg;
		for(Player perso : World.getOnlinePersos())
			send(perso,packet);
		if(Main.CONFIG_DEBUG)
	...
	{
		String packet = "cMK"+suffix+"|"+guid+"|"+name+"|"+msg;
		for(Player perso : World.getOnlinePersos())if(perso.isOnline())if(perso.get_compte() != null)if(perso.get_compte().get_gmLvl()>0)
			send(perso,packet);
		if(Main.CONFIG_DEBUG)
			GameServer.addToSockLog("Game: ALL("+World.getOnlinePersos().size()+"): Send>>"+packet);
	}
	...
	public static void GAME_SEND_CONSOLE_MESSAGE_TO_ADMIN(String mess,int minGmLvl)
	{
		...
		ArrayList<Player> perso = new ArrayList<Player>();
	...
	public static void GAME_SEND_OBJET_MOVE_PACKET(Player out,Item obj)
	{
		...
		if(obj.getPosition() != Variable.ITEM_POS_NO_EQUIPED)
	...
	public static void GAME_SEND_MESSAGE_TO_ALL(String msg,String color)
	{
		...
		for(Player P : World.getOnlinePersos())
		{
			send(P,packet);
		}
		if(Main.CONFIG_DEBUG)
	...
	public static void GAME_SEND_GDF_PACKET_TO_MAP(Maps map, Case cell)
	{
		...
		for(Player perso : map.getPersos())send(perso,packet);
		if(Main.CONFIG_DEBUG)
			GameServer.addToSockLog("Game: Map: Send>>"+packet);
	}
	...
	public static void GAME_SEND_gIB_PACKET(Player p)
	{
		String packet = "";
	...
	public static void GAME_SEND_gIG_PACKET(Player p, Guild g)
	{
		...
		long xpMax = World.getExpLevel(g.get_lvl()>=Main.MAX_LEVEL?Main.MAX_LEVEL:g.get_lvl()+1).guilde;
	...
	public static void GAME_SEND_EHl(Player out, ActionHouse seller,int templateID)
	{
	...
	public static void GAME_SEND_SUBAREA_LIST(Player player) {
		String packet = "al"+World.SubArea.getPacketList();
		send(player, packet);
	}
	
	public static void GAME_SEND_RESTRICTIONS(PrintWriter out, String rights) {
		String packet = "AR"+rights;
		send(out,packet);
    }
	
	public static void GAME_SEND_TB_PACKET(Player perso) {
